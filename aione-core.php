...
define ('OXO_BUILDER_FA_PATH' , plugin_dir_path( __FILE__ ) . 'tinymce/css/font-awesome.css' );
define ('OXO_BUILDER_TEMPLATES_PATH' , plugin_dir_path( __FILE__ ) . 'admin/page-builder/templates/' );

if( ! get_option( 'aione_disable_builder' ) ) {
	if ( is_admin() ) {
		// Load Page Builder Functionality
		require_once( plugin_dir_path( __FILE__ ) . 'admin/class-pagebuilder.php' );
		add_action( 'after_setup_theme', array( 'Oxo_Core_PageBuilder', 'get_instance' ) );
	}

	// Load shortocded parser
	require_once( plugin_dir_path( __FILE__ ) . 'admin/page-builder/classes/class-shortcodes-parser.php' );
	add_filter( 'the_content', array('Oxo_Core_Shortcodes_Parser', 'check_builder_elements' ));
}

if( ! class_exists( 'OxoCore_Plugin' ) ) {
	class OxoCore_Plugin {
		/**
		...
		private function __construct() {
			define('OXO_TINYMCE_URI', plugin_dir_url( __FILE__ ) . 'tinymce');
			define('OXO_TINYMCE_DIR', plugin_dir_path( __FILE__ ) .'tinymce');

			add_action('init', array(&$this, 'init'));
			add_action('admin_init', array(&$this, 'admin_init'));
			add_action('after_setup_theme', array(&$this, 'load_oxo_core_text_domain'));
			//add_action('admin_init', array(&$this, 'updater'));
			add_action('wp_ajax_oxo_shortcodes_popup', array(&$this, 'popup'));
		}
		 ...
		 * @return void
		 */		
		function load_oxo_core_text_domain() {
			load_plugin_textdomain( 'oxo-core', false, dirname( plugin_basename(__FILE__) ) . '/languages' );
		}
		...
		public static function animations( $args = array() ) {
			...
			if ( $args['type'] ) {
				...
				$animation_attribues['animation_class'] = 'oxo-animated';
		...
		public static function aione_link_pages() {		
			wp_link_pages( array(
				'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'oxo-core' ) . '</span>',
				'after'       => '</div>',
		...
		{
			if( is_admin() ) {
				$plugin_array['oxo_button'] = OXO_TINYMCE_URI . '/plugin.js';
			}
		...
		{
			// css
			wp_enqueue_style( 'oxo-popup', OXO_TINYMCE_URI . '/css/popup.css', false, OxoCore_Plugin::VERSION, 'all' );
			wp_enqueue_style( 'oxo-jquery.chosen', OXO_TINYMCE_URI . '/css/chosen.css', false, OxoCore_Plugin::VERSION, 'all' );
			wp_enqueue_style( 'fuision-font-awesome', OXO_TINYMCE_URI . '/css/font-awesome.css', false, OxoCore_Plugin::VERSION, 'all' );
			wp_enqueue_style( 'wp-color-picker' );

			// js
			wp_enqueue_script( 'jquery-ui-sortable' );
			wp_enqueue_script( 'oxo-jquery-livequery', OXO_TINYMCE_URI . '/js/jquery.livequery.js', false, OxoCore_Plugin::VERSION, false );
			wp_enqueue_script( 'oxo-jquery-appendo', OXO_TINYMCE_URI . '/js/jquery.appendo.js', false, OxoCore_Plugin::VERSION, false );
			//wp_enqueue_script( 'oxo-base64', OXO_TINYMCE_URI . '/js/base64.js', false, OxoCore_Plugin::VERSION, false );
			wp_enqueue_script( 'oxo-jquery.chosen', OXO_TINYMCE_URI . '/js/chosen.jquery.min.js', false, OxoCore_Plugin::VERSION, false );
			wp_enqueue_script( 'wp-color-picker' );

			wp_enqueue_script( 'oxo-popup', OXO_TINYMCE_URI . '/js/popup.js', false, OxoCore_Plugin::VERSION, true );

			// Developer mode
			$dev_mode = current_theme_supports( 'oxo_shortcodes_embed' );
			if( $dev_mode ) {
				$dev_mode = 'true';
			} else {
				$dev_mode = 'false';
			...
			wp_localize_script( 'oxo-popup', 'OxoShortcodes', array('plugin_folder' => plugins_url( '', __FILE__ ), 'dev' => $dev_mode) );
		}
		...
		function popup() {
			...
			require_once( OXO_TINYMCE_DIR . '/oxo-sc.php' );
		...
		/*function updater() {
			...
			$theme_update = new OxoCoreUpdater( 'http://updates.theme-oxo.com/oxo-core.php', $plugin_id, $plugin_slug );
		}*/
	}
}
// Load the instance of the plugin
add_action( 'plugins_loaded', array( 'OxoCore_Plugin', 'get_instance' ) );

// Blocking Script
if( ! function_exists( 'oxo_block_direct_access' ) ) {
	/**
	 * Blocks direct accessing of a core file
	 * @param  none
	 * @return void
	 */
	function oxo_block_direct_access() {
		if( ! defined( 'ABSPATH' ) ) {
 ...
 * Oxo Slider
 */
include_once 'aione-slider.php';

/*----------------------------------------------------------------------------*
 * Register custom post types
 *----------------------------------------------------------------------------*/
add_action( 'init', 'oxo_register_post_types' );
function oxo_register_post_types() {
	global $smof_data;
	...
	register_post_type(
		...
		array(
			'labels' => array(
				'name' 			=> _x( 'Portfolio', 'Post Type General Name', 'oxo-core' ),
				'singular_name' => _x( 'Portfolio', 'Post Type Singular Name', 'oxo-core' ),
			),
			'public' => true,
			'has_archive' => true,
			'rewrite' => array(
	...
	register_taxonomy('portfolio_category', 'aione_portfolio', 
		array(
			'hierarchical'	=> true, 
			'label' 		=> __('Portfolio Categories', 'oxo-core' ), 
			'query_var' 	=> true, 
			'rewrite'		=> array(
				'slug'       => empty( $permalinks['portfolio_category_base'] ) ? _x( 'portfolio_category', 'slug', 'oxo-core' ) : $permalinks['portfolio_category_base'],
				'with_front' => false
			),
		)
	);
	register_taxonomy('portfolio_skills', 'aione_portfolio', 
		array(
			'hierarchical' 	=> true, 
			'label' 		=> __( 'Skills', 'oxo-core' ),
			'query_var' 	=> true, 
			'rewrite'		=> array(
				'slug'       => empty( $permalinks['portfolio_skills_base'] ) ? _x( 'portfolio_skills', 'slug', 'oxo-core' ) : $permalinks['portfolio_skills_base'],
				'with_front' => false
			),
		)
	);
	register_taxonomy('portfolio_tags', 'aione_portfolio', 
		array(
			'hierarchical' => false, 
			'label' => __( 'Tags', 'oxo-core' ),
			'query_var' => true, 
			'rewrite'		=> array(
				'slug'       => empty( $permalinks['portfolio_tags_base'] ) ? _x( 'portfolio_tags', 'slug', 'oxo-core' ) : $permalinks['portfolio_tags_base'],
				'with_front' => false
			),
		)
	);
	...
	register_post_type(
		...
		array(
			'labels' => array(
				'name' 			=> _x( 'FAQs', 'Post Type General Name', 'oxo-core' ),
				'singular_name' => _x( 'FAQ', 'Post Type Singular Name', 'oxo-core' ),
			),
			'public' => true,
			'has_archive' => true,
	...
	register_taxonomy('faq_category', 'aione_faq', array('hierarchical' => true, 'label' => 'FAQ Categories', 'query_var' => true, 'rewrite' => true));
	...
	if( ! $smof_data['status_eslider'] ) {
		register_post_type(
			...
			array(
				...
				'menu_position' => 100,
				'labels' => array(
					'name'				=> _x( 'Elastic Sliders', 'Post Type General Name', 'oxo-core' ),
					'singular_name'	   => _x( 'Elastic Slider', 'Post Type Singular Name', 'oxo-core' ),
					'menu_name'		   => __( 'Elastic Slider', 'oxo-core' ),
		...
		register_taxonomy(
			...
			array(
				...
				'rewrite' => true,
				'labels' => array(
					'name'					   => _x( 'Groups', 'Taxonomy General Name', 'oxo-core' ),
					'singular_name'			  => _x( 'Group', 'Taxonomy Singular Name', 'oxo-core' ),
					'menu_name'				  => __( 'Add or Edit Groups', 'oxo-core' ),
					'all_items'				  => __( 'All Groups', 'oxo-core' ),
					'parent_item_colon'		  => __( 'Parent Group:', 'oxo-core' ),
					'new_item_name'			  => __( 'New Group Name', 'oxo-core' ),
					'add_new_item'			   => __( 'Add Groups', 'oxo-core' ),
					'edit_item'				  => __( 'Edit Group', 'oxo-core' ),
					'update_item'				=> __( 'Update Group', 'oxo-core' ),
					'separate_items_with_commas' => __( 'Separate groups with commas', 'oxo-core' ),
					'search_items'			   => __( 'Search Groups', 'oxo-core' ),
					'add_or_remove_items'		=> __( 'Add or remove groups', 'oxo-core' ),
					'choose_from_most_used'	  => __( 'Choose from the most used groups', 'oxo-core' ),
					'not_found'				  => __( 'Not Found', 'oxo-core' ),
				),
			)
		);
...
add_action( 'admin_menu', 'oxo_admin_menu' );
function oxo_admin_menu() {
	global $submenu;

	unset( $submenu['edit.php?post_type=themeoxo_elastic'][10] );
}

add_action( 'wp_head', 'oxo_compatibility_function' );
function oxo_compatibility_function() {
...
add_action('admin_print_footer_scripts','oxo_add_quicktags_button');

function oxo_add_quicktags_button() {
	if( get_current_screen()->base == 'post' ) {
	?>
		<script type="text/javascript" charset="utf-8">
			if ( typeof( QTags ) == 'function' ) {
				QTags.addButton( 'oxo_shortcodes_text_mode', ' ','', '', 'f' );
			}
		</script>
	<?php
	}
...
add_filter('the_content', 'aione_shortcodes_formatter');
add_filter('widget_text', 'aione_shortcodes_formatter');
