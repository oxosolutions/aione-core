...
if( ! class_exists( 'Oxo_Slider' ) ) {
	class Oxo_Slider {
		...
		function init() {
			...
			if( ! $smof_data['status_oxo_slider'] ) {
				register_post_type(
					'slide',
					array(
						'public' => true,
						...
						'hierarchical' => false,
						'labels' => array(
							'name'				=> _x( 'Slides', 'Post Type General Name', 'oxo-core' ),
							'singular_name'	   => _x( 'Slide', 'Post Type Singular Name', 'oxo-core' ),
							'menu_name'		   => __( 'Slider', 'oxo-core' ),
							'parent_item_colon'   => __( 'Parent Slide:', 'oxo-core' ),
							'all_items'		   => __( 'Add or Edit Slides', 'oxo-core' ),
							'view_item'		   => __( 'View Slides', 'oxo-core' ),
							'add_new_item'		=> __( 'Add New Slide', 'oxo-core' ),
							'add_new'			 => __( 'Add New Slide', 'oxo-core' ),
							'edit_item'		   => __( 'Edit Slide', 'oxo-core' ),
							'update_item'		 => __( 'Update Slide', 'oxo-core' ),
							'search_items'		=> __( 'Search Slide', 'oxo-core' ),
							'not_found'		   => __( 'Not found', 'oxo-core' ),
							'not_found_in_trash'  => __( 'Not found in Trash', 'oxo-core' ),
						)
					)
				);
				...
				register_taxonomy('slide-page', 'slide',
					array(
						...
						'show_tagcloud' => false,
						'labels' => array(
							'name'					   => __( 'Sliders', 'oxo-core' ),
							'singular_name'			  => __( 'Slider', 'oxo-core' ),
							'menu_name'				  => __( 'Add or Edit Sliders', 'oxo-core' ),
							'all_items'				  => __( 'All Sliders', 'oxo-core' ),
							'parent_item_colon'		  => __( 'Parent Slider:', 'oxo-core' ),
							'new_item_name'			  => __( 'New Slider Name', 'oxo-core' ),
							'add_new_item'			   => __( 'Add Slider', 'oxo-core' ),
							'edit_item'				  => __( 'Edit Slider', 'oxo-core' ),
							'update_item'				=> __( 'Update Slider', 'oxo-core' ),
							'separate_items_with_commas' => __( 'Separate sliders with commas', 'oxo-core' ),
							'search_items'			   => __( 'Search Sliders', 'oxo-core' ),
							'add_or_remove_items'		=> __( 'Add or remove sliders', 'oxo-core' ),
							'choose_from_most_used'	  => __( 'Choose from the most used sliders', 'oxo-core' ),
							'not_found'				  => __( 'Not Found', 'oxo-core' ),
						),
					)
				);
		...
		function admin_init() {
			...
			if( ( isset( $_GET['taxonomy'] ) && $_GET['taxonomy'] == 'slide-page' ) || ( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'slide' ) || $post_type == 'slide' ) {
				wp_enqueue_script( 'oxo-slider', plugin_dir_url( __FILE__ ) . 'js/oxo-slider.js', false, '1.0', true );
			}
		...
		function admin_menu() {
			...
			add_submenu_page( 'edit.php?post_type=slide', __( 'Export / Import', 'oxo-core' ), __( 'Export / Import', 'oxo-core' ), 'manage_options', 'fs_export_import', array( $this, 'fs_export_import_settings' ) );
		}
		...
		function slider_add_new_meta_fields() {
			...
			?>
			<div class="form-field oxo-double-fields">
				<label for="term_meta[slider_width]"><?php _e( 'Slider Size', 'oxo-core' ); ?></label>
				<p class="description"><?php _e( 'Enter a pixel value for width and height, ex: 1000px', 'oxo-core' ); ?></p>

				<div class="oxo-field">
					<input type="text" name="term_meta[slider_width]" id="term_meta[slider_width]" value="100%">
					<label for="term_meta[slider_width]"><?php _e( 'Width', 'oxo-core' ); ?></label>
				</div>
				<div class="oxo-field">
					<input type="text" name="term_meta[slider_height]" id="term_meta[slider_height]" value="400px">
					<label for="term_meta[slider_height]"><?php _e( 'Height', 'oxo-core' ); ?></label>
				</div>
			</div>
			<div class="form-field">
				<label for="term_meta[slider_content_width]"><?php _e( 'Slider Content Max Width', 'oxo-core' ); ?></label>
				<input type="text" name="term_meta[slider_content_width]" id="term_meta[slider_content_width]" value="">
				<p class="description"><?php _e( 'Controls the width of content, In pixels, ex: 850px. Leave blank for site width.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field form-field-checkbox">
				<label for="term_meta[full_screen]"><?php _e( 'Full Screen Slider', 'oxo-core' ); ?></label>
				...
				<p class="description"><?php _e( 'Check this option if you want full width and height of the screen.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field form-field-checkbox">
				<label for="term_meta[parallax]"><?php _e( 'Parallax Scrolling Effect', 'oxo-core' ); ?></label>
				...
				<p class="description"><?php _e( 'Check this box to have a parallax scrolling effect, this ONLY works when assigning the slider in page options. It does not work when using a slider shortcode. With this option enabled, the slider height you input will not be exact due to negative margin which is based off the overall header size. ex: 500px will show as 415px. Please adjust accordingly.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field form-field-checkbox">
				<label for="term_meta[nav_arrows]"><?php _e( 'Display Navigation Arrows', 'oxo-core' ); ?></label>
				...
				<p class="description"><?php _e( 'Check this box to display the navigation arrows.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field oxo-double-fields">
				<label for="term_meta[nav_box_width]"><?php _e( 'Navigation Box Size', 'oxo-core' ); ?></label>
				<p class="description"><?php _e( 'Enter a pixel value for width and height, ex: 40px', 'oxo-core' ); ?></p>
				<div class="oxo-field">
					<input type="text" name="term_meta[nav_box_width]" id="term_meta[nav_box_width]" value="63px">
					<label for="term_meta[nav_box_width]"><?php _e( 'Width', 'oxo-core' ); ?></label>
				</div>
				<div class="oxo-field">
					<input type="text" name="term_meta[nav_box_height]" id="term_meta[nav_box_height]" value="63px">
					<label for="term_meta[nav_box_height]"><?php _e( 'Height', 'oxo-core' ); ?></label>
				</div>
			</div>
			<div class="form-field">
				<label for="term_meta[nav_arrow_size]"><?php _e( 'Navigation Arrow Size', 'oxo-core' ); ?></label>
				<input type="text" name="term_meta[nav_arrow_size]" id="term_meta[nav_arrow_size]" value="25px">
				<p class="description"><?php _e( 'Enter a pixel value for the arrow size, ex: 14px', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field form-field-checkbox">
				<label for="term_meta[pagination_circles]"><?php _e( 'Display Pagination Circles', 'oxo-core' ); ?></label>
				...
				<p class="description"><?php _e( 'Check this box to display the pagination circles.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field form-field-checkbox">
				<label for="term_meta[autoplay]"><?php _e( 'Autoplay', 'oxo-core' ); ?></label>
				...
				<p class="description"><?php _e( 'Check this box to autoplay the slides.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field form-field-checkbox">
				<label for="term_meta[loop]"><?php _e( 'Slide Loop', 'oxo-core' ); ?></label>
				...
				<p class="description"><?php _e( 'Check this box to have the slider loop infinitely.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field">
				<label for="term_meta[animation]"><?php _e( 'Animation', 'oxo-core' ); ?></label>
				<select name="term_meta[animation]" id="term_meta[animation]">
					<option value="fade">Fade</option>
					<option value="slide">Slide</option>
				</select>
				<p class="description"><?php _e( 'The type of animation when slides rotate.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field">
				<label for="term_meta[slideshow_speed]"><?php _e( 'Slideshow Speed', 'oxo-core' ); ?></label>
				<input type="text" name="term_meta[slideshow_speed]" id="term_meta[slideshow_speed]" value="7000">
				<p class="description"><?php _e( 'Controls the speed of the slideshow. 1000 = 1 second.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field">
				<label for="term_meta[animation_speed]"><?php _e( 'Animation Speed', 'oxo-core' ); ?></label>
				<input type="text" name="term_meta[animation_speed]" id="term_meta[animation_speed]" value="600">
				<p class="description"><?php _e( 'Controls the speed of the slide transition from slide to slide. 1000 = 1 second.', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field">
				<label for="term_meta[typo_sensitivity]"><?php _e( 'Responsive Typography Sensitivity', 'oxo-core' ); ?></label>
				<input type="text" name="term_meta[typo_sensitivity]" id="term_meta[typo_sensitivity]" value="1">
				<p class="description"><?php _e( 'Values below 1 decrease resizing, values above 1 increase sizing. ex: .6', 'oxo-core' ); ?></p>
			</div>
			<div class="form-field">
				<label for="term_meta[typo_factor]"><?php _e( 'Mininum Font Size Factor', 'oxo-core' ); ?></label>
				<input type="text" name="term_meta[typo_factor]" id="term_meta[typo_factor]" value="1.5">
				<p class="description"><?php _e( 'Minimum font factor is used to determine minimum distance between headings and body type by a multiplying value. ex: 1.5', 'oxo-core' ); ?></p>
			</div>
		<?php
		}
		...
		function slider_edit_meta_fields( $term ) {
			...
			?>
			<tr class="form-field oxo-double-fields">
				<th scope="row" valign="top"><label for="term_meta[slider_width]"><?php _e( 'Slider Size', 'oxo-core' ); ?></label></th>
				<td>
					<p class="description"><?php _e( 'Enter a pixel value for width and height, ex: 1000px', 'oxo-core' ); ?></p>
					<div class="oxo-field">
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[slider_content_width]"><?php _e( 'Slider Content Max Width', 'oxo-core' ); ?></label></th>
				<td>
			...
			<tr class="form-field form-field-checkbox">
				<th scope="row" valign="top"><label for="term_meta[full_screen]"><?php _e( 'Full Screen Slider', 'oxo-core' ); ?></label></th>
				<td>
					<input type="hidden" name="term_meta[full_screen]" id="term_meta[full_screen]" value="0">
			...
			<tr class="form-field form-field-checkbox">
				<th scope="row" valign="top"><label for="term_meta[parallax]"><?php _e( 'Parallax Scrolling Effect', 'oxo-core' ); ?></label></th>
				<td>
					<input type="hidden" name="term_meta[parallax]" id="term_meta[parallax]" value="0">
			...
			<tr class="form-field form-field-checkbox">
				<th scope="row" valign="top"><label for="term_meta[nav_arrows]"><?php _e( 'Display Navigation Arrows', 'oxo-core' ); ?></label></th>
				<td>
					<input type="hidden" name="term_meta[nav_arrows]" id="term_meta[nav_arrows]" value="0">
			...
			<tr class="form-field oxo-double-fields">
				<th scope="row" valign="top"><label for="term_meta[nav_box_width]"><?php _e( 'Navigation Box Size', 'oxo-core' ); ?></label></th>
				<td>
					<p class="description"><?php _e( 'Enter a pixel value for width and height, ex: 40px', 'oxo-core' ); ?></p>
					<div class="oxo-field">
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[nav_arrow_size]"><?php _e( 'Navigation Arrow Size', 'oxo-core' ); ?></label></th>
				<td>
			...
			<tr class="form-field form-field-checkbox">
				<th scope="row" valign="top"><label for="term_meta[pagination_circles]"><?php _e( 'Display Pagination Circles', 'oxo-core' ); ?></label></th>
				<td>
					<input type="hidden" name="term_meta[pagination_circles]" id="term_meta[pagination_circles]" value="0">
			...
			<tr class="form-field form-field-checkbox">
				<th scope="row" valign="top"><label for="term_meta[autoplay]"><?php _e( 'Autoplay', 'oxo-core' ); ?></label></th>
				<td>
					<input type="hidden" name="term_meta[autoplay]" id="term_meta[autoplay]" value="0">
			...
			<tr class="form-field form-field-checkbox">
				<th scope="row" valign="top"><label for="term_meta[loop]"><?php _e( 'Slide Loop', 'oxo-core' ); ?></label></th>
				<td>
					<input type="hidden" name="term_meta[loop]" id="term_meta[loop]" value="0">
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[animation]"><?php _e( 'Animation', 'oxo-core' ); ?></label></th>
				<td>
					<select name="term_meta[animation]" id="term_meta[animation]">
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[slideshow_speed]"><?php _e( 'Slideshow Speed', 'oxo-core' ); ?></label></th>
				<td>
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[animation_speed]"><?php _e( 'Animation Speed', 'oxo-core' ); ?></label></th>
				<td>
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[typo_sensitivity]"><?php _e( 'Responsive Typography Sensitivity', 'oxo-core' ); ?></label></th>
				<td>
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label for="term_meta[typo_factor]"><?php _e( 'Mininum Font Size Factor', 'oxo-core' ); ?></label></th>
				<td>
		...
		?>
		<div class="wrap">
			<h2><?php _e( 'Export and Import Oxo Sliders', 'oxo-core' ); ?></h2>
			<form enctype="multipart/form-data" method="post" action="">
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e( 'Export', 'oxo-core' ); ?></th>
						<td><input type="submit" class="button button-primary" name="export_button" value="<?php _e( 'Export All Sliders', 'oxo-core' ); ?>" /></td>
					</tr>
					<tr valign="top">
						<th>
							<label for="upload"><?php _e( 'Choose a file from your computer:', 'oxo-core'); ?></label>
						</th>
						<td>
			...
			if( isset($_POST['export_button']) && $_POST['export_button'] ) {
				...
				if( WP_Filesystem( $creds ) ) {
					...
					if ( ! $wp_filesystem->put_contents( $fs_dir . 'sliders.xml', $export, FS_CHMOD_FILE ) || ! $wp_filesystem->put_contents( $fs_dir . 'settings.json', $json_export_terms, FS_CHMOD_FILE ) ) {
						...
						$zip = new ZipArchive;
						$zip->open( 'oxo_slider.zip', ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE );
						...
						header( 'Content-Disposition: attachment; filename="oxo_slider.zip"' );
		...
		function import_sliders( $zip_file ) {
			$upload_dir = wp_upload_dir();
			$base_dir = trailingslashit( $upload_dir['basedir'] );
			$fs_dir = $base_dir . 'oxo_slider_exports/';
