...
class Aione_Compatibility {
	public $settings;
	public function __construct() {
		$this->settings = new Aione_Compatibility_Settings();
		
		require_once( 'class-aione-sanitize.php' );
	}
}

class Aione_Compatibility_Settings {
	public function get( $setting, $subsetting = false ) {
		$settings = get_option( 'Aione_options' );
...
if ( ! function_exists( 'Aione' ) ) {
	function Aione() {
		$aione = new Aione_Compatibility();
		return $aione;
	}
...
if( ! function_exists( 'oxo_get_related_posts' ) ) {
	...
	function oxo_get_related_posts( $post_id, $number_posts = -1 ) {
		...
		if ( ! Aione()->settings->get( 'featured_image_placeholder' ) ) {
...
if( ! function_exists( 'oxo_pagination' ) ) {
	...
	function oxo_pagination( $pages = '', $range = 2, $current_query = '' ) {
		...
		if( $pages == '' ) {
		 ...
		 if(1 != $pages)
		 {
			if ( ( Aione()->settings->get( 'blog_pagination_type' ) != 'Pagination' && ( is_home() || is_search() || ( get_post_type() == 'post' && ( is_author() || is_archive() ) ) ) ) ||
				 ( Aione()->settings->get( 'grid_pagination_type' ) != 'Pagination' && ( aione_is_portfolio_template() || is_post_type_archive( 'aione_portfolio' ) || is_tax( 'portfolio_category' ) || is_tax( 'portfolio_skills' )  || is_tax( 'portfolio_tags' ) ) )
			) {
...
if ( ! function_exists( 'oxo_get_post_content' ) ) {
	...
	function oxo_get_post_content( $page_id = '', $excerpt = 'blog', $excerpt_length = 55, $strip_html = FALSE ) {
		...
		if ( $excerpt == 'blog' ) {
			...
			if ( strtolower( oxo_get_theme_option( 'content_length' ) ) == 'excerpt' ) {
				$content_excerpted = TRUE;

				// Get the excerpt length
				$excerpt_length = oxo_get_theme_option( 'excerpt_length_blog' );
			}

			// Check if HTML should be stripped from contant
			if ( oxo_get_theme_option( 'strip_html_excerpt' ) ) {
				$strip_html = TRUE;
			}
		...
		// Portfolio page templates
			...
			if ( oxo_get_option( 'portfolio_content_length', 'portfolio_content_length', $page_id ) == 'excerpt' ) {
				...
				if ( oxo_get_page_option( 'portfolio_excerpt', $page_id ) ) {
					...
					$excerpt_length =  oxo_get_theme_option( 'excerpt_length_portfolio' );
				}
			} else if ( ! $page_id &&
						oxo_get_theme_option( 'portfolio_content_length' ) == 'Excerpt'
			) {
				$content_excerpted = TRUE;
				$excerpt_length =  oxo_get_theme_option( 'excerpt_length_portfolio' );
			}
			...
			// Check if HTML should be stripped from contant
			if ( oxo_get_theme_option( 'portfolio_strip_html_excerpt' ) ) {
				$strip_html = TRUE;
			}
		// Shortcodes
		} elseif( $excerpt == 'yes' ) {
...
if ( ! function_exists( 'oxo_get_post_content_excerpt' ) ) {
	...
	function oxo_get_post_content_excerpt( $limit, $strip_html ) {
		...
		if ( ( $raw_content && $custom_excerpt == FALSE )
			...
			if ( Aione()->settings->get( 'excerpt_base' ) == 'Characters' ) {
				$content = mb_substr($content, 0, $limit);
				if ( $limit != 0 &&
					! Aione()->settings->get( 'disable_excerpts' )
				) {
					$content .= $read_more;
				}
...
if ( ! function_exists( 'aione_render_blog_post_content' ) ) {
	function aione_render_blog_post_content() {
		if ( is_search() && Aione()->settings->get( 'search_excerpt' ) ) {
			return;
		}
...
if ( ! function_exists( 'aione_render_blog_post_date' ) ) {
	function aione_render_blog_post_date() { ?>
		<div class="oxo-date-box">
			<span class="oxo-date"><?php echo get_the_time( Aione()->settings->get( 'alternate_date_format_day' ) ); ?></span>
			<span class="oxo-month-year"><?php echo get_the_time( Aione()->settings->get( 'alternate_date_format_month_year' ) ); ?></span>
		</div>
		<?php
	}
}
	...
	function aione_render_author_info() {
		...
		<div class="oxo-author">
			...
			<div class="oxo-author-info">
				...
				<?php if ( ! Aione()->settings->get( 'disable_date_rich_snippet_pages' ) ) : ?>
			...
			<div class="oxo-author-social clearfix">
				...
				$author_soical_icon_options = array (
					...
					'linktarget'		=> Aione()->settings->get( 'social_icons_new' ),
				);
...
if ( ! function_exists( 'aione_render_footer_copyright_notice' ) ) {
	function aione_render_footer_copyright_notice() { ?>
		<div class="oxo-copyright-notice">
			<div><?php echo do_shortcode( Aione()->settings->get( 'footer_text' ) ); ?></div>
		</div>
		<?php
	}
}
...
if ( ! function_exists( 'aione_render_footer_social_icons' ) ) {
	function aione_render_footer_social_icons() {
		...
		if ( Aione()->settings->get( 'icons_footer' ) ) : ?>
			<div class="oxo-social-links-footer">
				<?php

				$footer_soical_icon_options = array (
					'position'          => 'footer',
					'icon_colors'       => Aione()->settings->get( 'footer_social_links_icon_color' ),
					'box_colors'        => Aione()->settings->get( 'footer_social_links_box_color' ),
					'icon_boxed'        => Aione()->settings->get( 'footer_social_links_boxed' ),
					'icon_boxed_radius' => Aione()->settings->get( 'footer_social_links_boxed_radius' ),
					'tooltip_placement' => Aione()->settings->get( 'footer_social_links_tooltip_placement' ),
					'linktarget'        => Aione()->settings->get( 'social_icons_new' ),
				);
...
if ( ! function_exists( 'aione_render_rollover' ) ) {
	function aione_render_rollover( $post_id, $post_permalink = '', $display_woo_price = false, $display_woo_buttons = false, $display_post_categories = 'default', $display_post_title = 'default', $gallery_id = '', $display_woo_rating = false ) {
		...
		if ( 'enable' == $display_post_categories ) {
			...
			$display_post_categories = ! Aione()->settings->get( 'cats_image_rollover' );
		}
		...
		if ( 'enable' == $display_post_title ) {
			...
			$display_post_title = ! Aione()->settings->get( 'title_image_rollover' );
		}
		...
		if ( '' == oxo_get_page_option( 'image_rollover_icons', $post_id ) || 'default' == oxo_get_page_option( 'image_rollover_icons', $post_id ) ) {
			if( ! Aione()->settings->get( 'link_image_rollover' ) && ! Aione()->settings->get( 'zoom_image_rollover' ) ) { // link + zoom
				$image_rollover_icons = 'linkzoom';
			} elseif( ! Aione()->settings->get( 'link_image_rollover' ) && Aione()->settings->get( 'zoom_image_rollover' ) ) { // link
				$image_rollover_icons = 'link';
			} elseif( Aione()->settings->get( 'link_image_rollover' ) && ! Aione()->settings->get( 'zoom_image_rollover' ) ) { // zoom
		...
		if ( 'yes' == oxo_get_page_option( 'link_icon_target', $post_id ) ||
			 ...
			 ( 'aione_portfolio' == get_post_type() &&  Aione()->settings->get( 'portfolio_link_icon_target' ) && 'default' == oxo_get_page_option( 'link_icon_target', $post_id ) )
		) {
				...
				<?php if ( 'no' != $image_rollover_icons && 'product' != get_post_type( $post_id ) ) : // Check if rollover icons should be displayed ?>
					...
					<?php if ( 'link' != $image_rollover_icons ) : // If set, render the rollover zoom icon ?>
						...
						<?php if ( $full_image[0] ) : // Render the rollover zoom icon if we have an image ?>
							...
							if ( 'individual' == Aione()->settings->get( 'lightbox_behavior' ) ) {
...
if ( ! function_exists( 'aione_render_first_featured_image_markup' ) ) {
	...
	function aione_render_first_featured_image_markup( $post_id, $post_featured_image_size = '', $post_permalink = '', $display_placeholder_image = FALSE, $display_woo_price = FALSE, $display_woo_buttons = FALSE, $display_post_categories = 'default', $display_post_title = 'default', $type = '', $gallery_id = '', $display_rollover = 'yes', $display_woo_rating = FALSE ) {
		...
		$html = '<div class="oxo-image-wrapper' . $image_size_class . '" aria-haspopup="true">';
			...
			if ( ( Aione()->settings->get( 'image_rollover' ) && $display_rollover == 'yes' ) ||
...
if ( ! function_exists( 'aione_render_post_title' ) ) {
	...
	function aione_render_post_title( $post_id = '', $linked = TRUE, $custom_title = '', $custom_size = '2' ) {
		...
		if ( ! Aione()->settings->get( 'disable_date_rich_snippet_pages' ) ) {
...
if ( ! function_exists( 'aione_get_portfolio_image_size' ) ) {
	function aione_get_portfolio_image_size( $current_page_id ) {
		...
		if ( get_post_meta( $current_page_id, 'pyre_portfolio_featured_image_size', true ) == 'default' ||
			...
			if ( 'full' == Aione()->settings->get( 'portfolio_featured_image_size' ) ) {
...
if ( ! function_exists( 'aione_get_blog_layout' ) ) {
	...
	function aione_get_blog_layout() {
		...
		$blog_layout = str_replace( ' ', '-', strtolower( Aione()->settings->get( $theme_options_blog_var ) ) );
...
if ( ! function_exists( 'aione_render_post_metadata' ) ) {
	...
	function aione_render_post_metadata( $layout, $settings = array() ) {
		...
		if ( ! $settings ) {
			$settings['post_meta']          = Aione()->settings->get( 'post_meta' );
			$settings['post_meta_author']   = Aione()->settings->get( 'post_meta_author' );
			$settings['post_meta_date']     = Aione()->settings->get( 'post_meta_date' );
			$settings['post_meta_cats']     = Aione()->settings->get( 'post_meta_cats' );
			$settings['post_meta_tags']     = Aione()->settings->get( 'post_meta_tags' );
			$settings['post_meta_comments'] = Aione()->settings->get( 'post_meta_comments' );
		}
		...
		if ( ( $settings['post_meta'] && get_post_meta( get_queried_object_id(), 'pyre_post_meta', TRUE ) != 'no' ) ||
			...
			if ( ! $settings['post_meta_date'] ) {
				...
				$metadata .= sprintf( '<span>%s</span><span class="oxo-inline-sep">|</span>', get_the_time( Aione()->settings->get( 'date_format' ) ) );
			} else {
	...
	function aione_render_social_sharing( $post_type = 'post' ) {
		...
		if ( ( Aione()->settings->get( $setting_name ) && get_post_meta( get_the_ID(), 'pyre_share_box', true) != 'no' ) ||
			...
			$sharingbox_soical_icon_options = array (
				'sharingbox'		=> 'yes',
				'icon_colors' 		=> Aione()->settings->get( 'sharing_social_links_icon_color' ),
				'box_colors' 		=> Aione()->settings->get( 'sharing_social_links_box_color' ),
				'icon_boxed' 		=> Aione()->settings->get( 'sharing_social_links_boxed' ),
				'icon_boxed_radius' => Aione()->settings->get( 'sharing_social_links_boxed_radius' ),
				'tooltip_placement'	=> Aione()->settings->get( 'sharing_social_links_tooltip_placement' ),
				'linktarget'        => Aione()->settings->get( 'social_icons_new' ),
			...
			<div class="oxo-sharing-box oxo-single-sharing-box share-box">
				<h4><?php echo apply_filters( 'oxo_sharing_box_tagline', Aione()->settings->get( 'sharing_social_tagline' ) ); ?></h4>
...
if( ! function_exists( 'aione_render_related_posts' ) ) {
	...
	function aione_render_related_posts( $post_type = 'post' ) {
		...
		if ( oxo_get_option( $theme_option_name, 'related_posts', get_the_ID() ) == 'yes' ||
			 oxo_get_option( $theme_option_name, 'related_posts', get_the_ID() ) == '1'
		) {
			if ( $post_type == 'post' ) {
				$related_posts = oxo_get_related_posts( get_the_ID(), Aione()->settings->get( 'number_related_posts' ) );
			} elseif ( $post_type == 'aione_portfolio' ) {
				$related_posts = oxo_get_related_projects( get_the_ID(), Aione()->settings->get( 'number_related_posts' ) );
			}
			...
			if ( $related_posts->have_posts() ) {
				$html .= '<div class="related-posts single-related-posts">';
					...
					if ( 'cropped' == Aione()->settings->get( 'related_posts_image_size' ) ) {
					...
					if ( 'title_on_rollover' == Aione()->settings->get( 'related_posts_layout' ) ) {
					...
					if ( Aione()->settings->get( 'related_posts_autoplay' ) ) {
						$data_autoplay = 'yes';
					} else {
						$data_autoplay = 'no';
					}
					...
					if ( Aione()->settings->get( 'related_posts_swipe' ) ) {
						$data_swipe = 'yes';
					} else {
						$data_swipe = 'no';
					}
					...
					$carousel_item_css = '';
					if ( sizeof( $related_posts->posts ) < Aione()->settings->get( 'related_posts_columns' ) ) {
					...
					$html .= sprintf( '<div class="oxo-carousel" data-imagesize="%s" data-metacontent="%s" data-autoplay="%s" data-touchscroll="%s" data-columns="%s" data-itemmargin="%s" data-itemwidth="180" data-touchscroll="yes" data-scrollitems="%s">',
									  $data_image_size, $data_meta_content, $data_autoplay, $data_swipe, Aione()->settings->get( 'related_posts_columns' ), Aione()->settings->get( 'related_posts_column_spacing' ), Aione()->settings->get( 'related_posts_swipe_items' ) );
						$html .= '<div class="oxo-carousel-positioner">';
										...
										$html .= '<div class="oxo-carousel-item-wrapper">';
											...
											// Title below image layout
												...
												$html .= '<div class="oxo-carousel-meta">';
													...
													$html .= sprintf( '<span class="oxo-date">%s</span>', get_the_time( Aione()->settings->get( 'date_format' ), get_the_ID() ) );
							...
							if ( Aione()->settings->get( 'related_posts_navigation' ) ) {
...
if ( ! function_exists( 'aione_page_title_bar' ) ) {
	...
	function aione_page_title_bar( $title, $subtitle, $secondary_content ) {
		...
		} elseif ( Aione()->settings->get( 'page_title_alignment' ) ) {
			$alignment = Aione()->settings->get( 'page_title_alignment' );
		}
				...
				<div class="oxo-page-title-wrapper">
					<div class="oxo-page-title-captions">
						<?php if ( $title ) : ?>
							...
							<?php $entry_title_class = ( ! Aione()->settings->get( 'disable_date_rich_snippet_pages' ) ) ? ' class="entry-title"' : ''; ?>
						...
						<?php if ( 'center' == $alignment ) : // Render secondary content on center layout ?>
							<?php if ( 'none' != oxo_get_option( 'page_title_bar_bs', 'page_title_breadcrumbs_search_bar', $post_id ) ) : ?>
		...
		if( $ubermenu == false ) {
			if( $args->theme_location == 'main_navigation' || $args->theme_location == 'top_navigation' || $args->theme_location == 'sticky_navigation' ) {
				if( $args->theme_location == 'main_navigation' || $args->theme_location == 'sticky_navigation' ) {
					$is_enabled = oxo_get_theme_option( 'woocommerce_acc_link_main_nav' );
				} else if( $args->theme_location == 'top_navigation' ) {
					$is_enabled = oxo_get_theme_option( 'woocommerce_acc_link_top_nav' );
				}
				...
				if( class_exists( 'WooCommerce' ) && $is_enabled ) {
					...
					if ( $woo_account_page_link ) {
						$items .= '<li class="oxo-custom-menu-item oxo-menu-login-box">';
							...
							$caret_icon = '';
							if ( Aione()->settings->get( 'menu_display_dropdown_indicator' ) ) {
								$caret_icon = '<span class="oxo-caret"><i class="oxo-dropdown-indicator"></i></span>';
							}
							if ( 'Right' == Aione()->settings->get( 'header_position' ) ) {
...
if( ! function_exists( 'aione_nav_woo_cart' ) ) {
	...
	function aione_nav_woo_cart( $position = 'main' ) {
		...
		if( $position == 'main' ) {
			$is_enabled = oxo_get_theme_option( 'woocommerce_cart_link_main_nav' );
			...
			if( Aione()->settings->get( 'woocommerce_cart_counter') ) {
			...
			if( ! Aione()->settings->get( 'woocommerce_cart_counter') && $woocommerce->cart->get_cart_contents_count() ) {
			...
			$is_enabled = oxo_get_theme_option( 'woocommerce_cart_link_top_nav' );
...
if( ! function_exists( 'aione_add_search_to_main_nav' ) ) {
	function aione_add_search_to_main_nav( $items, $args ) {
		...
		if( $ubermenu == false ) {
			if( $args->theme_location == 'main_navigation'  || $args->theme_location == 'sticky_navigation' ) {
				if( oxo_get_theme_option( 'main_nav_search_icon' ) ) {
...
if ( ! function_exists( 'aione_set_post_views' ) ) {
...
if ( ! function_exists( 'aione_revslider' ) ) {
...
if ( ! function_exists( 'aione_layerslider' ) ) {
	function aione_layerslider( $id ) {
		...
		<div id="layerslider-container">
			<div id="layerslider-wrapper">
				<?php if ( 'aione' == $ls_slider['properties']['skin'] ) : ?>
...
if ( ! function_exists( 'aione_elasticslider' ) ) {
	function aione_elasticslider( $term ) {

		if ( ! Aione()->settings->get( 'status_eslider' ) ) {
			$args				= array(
				'post_type'        => 'themeoxo_elastic',
				'posts_per_page'   => -1,
				'suppress_filters' => 0
			);
			$args['tax_query'][] = array(
				'taxonomy' => 'themeoxo_es_groups',
				'field'    => 'slug',
				'terms'    => $term
			);
			$query = new WP_Query( $args );
			...
			<?php if ( $query->have_posts() ) : ?>
				<div id="ei-slider" class="ei-slider">
					<div class="oxo-slider-loading"><?php _e( 'Loading...', 'Aione' ); ?></div>
...
if ( ! function_exists( 'aione_wooslider' ) ) {
	function aione_wooslider( $term ) {

		if ( ! Aione()->settings->get( 'status_oxo_slider' ) ) {
			...
			<?php if ( $query->have_posts() ) : ?>
				...
				<div class="oxo-slider-container oxo-slider-<?php the_ID(); ?> <?php echo $slider_class; ?>-container" style="height:<?php echo $slider_settings['slider_height']; ?>;max-width:<?php echo $slider_settings['slider_width']; ?>;">
					<style type="text/css" scoped="scoped">
					.oxo-slider-<?php the_ID(); ?> .flex-direction-nav a {
						<?php
							...
							<?php while ( $query->have_posts() ) : $query->the_post(); ?>
								...
								if ( isset( $metadata['pyre_heading_bg'][0] ) && 'yes' == $metadata['pyre_heading_bg'][0] ) {
									...
									if ( isset( $metadata['pyre_heading_bg_color'][0] ) && '' != $metadata['pyre_heading_bg_color'][0] ) {
										$rgb        = oxo_hex2rgb( $metadata['pyre_heading_bg_color'][0] );
								...
								if ( isset( $metadata['pyre_caption_bg'][0] ) && 'yes' == $metadata['pyre_caption_bg'][0] ) {
									...
									if ( isset( $metadata['pyre_caption_bg_color'][0] ) && '' != $metadata['pyre_caption_bg_color'][0] ) {
										$rgb        = oxo_hex2rgb( $metadata['pyre_caption_bg_color'][0] );
								...
								if ( isset( $metadata['pyre_video_bg_color'][0] ) && $metadata['pyre_video_bg_color'][0] ) {
									$video_bg_color_hex = oxo_hex2rgb( $metadata['pyre_video_bg_color'][0]  );
								...
								if ( $metadata['pyre_content_alignment'][0] == 'center' ) {
									if ( $metadata['pyre_heading_separator'][0] != 'none' ) {
										$heading_title_sc_wrapper_class = ' oxo-block-element';
									}
									
									if ( $metadata['pyre_caption_separator'][0] != 'none' ) {
										$caption_title_sc_wrapper_class = ' oxo-block-element';
									}
								}
								?>
								<li data-mute="<?php echo $data_mute; ?>" data-loop="<?php echo $data_loop; ?>" data-autoplay="<?php echo $data_autoplay; ?>">
										...
										<div class="slide-content" style="<?php echo $content_max_width; ?>">
											<?php if ( isset( $metadata['pyre_heading'][0] ) && $metadata['pyre_heading'][0] ) : ?>
												<div class="heading <?php echo ( $heading_bg ) ? 'with-bg' : ''; ?>">
													<div class="oxo-title-sc-wrapper<?php echo $heading_title_sc_wrapper_class; ?>" style="<?php echo $heading_bg; ?>">
											...
											<?php if ( isset( $metadata['pyre_caption'][0] ) && $metadata['pyre_caption'][0] ) : ?>
												<div class="caption <?php echo ( $caption_bg ) ? 'with-bg' : ''; ?>">
													<div class="oxo-title-sc-wrapper<?php echo $caption_title_sc_wrapper_class; ?>" style="<?php echo $caption_bg; ?>">
									...
									<?php if ( isset( $metadata['pyre_preview_image'][0] ) && $metadata['pyre_preview_image'][0] && isset( $metadata['pyre_type'][0] ) && 'self-hosted-video' == $metadata['pyre_type'][0] ) : ?>
										<div class="mobile_video_image" style="background-image: url(<?php echo Aione_Sanitize::css_asset_url( $metadata['pyre_preview_image'][0] ); ?>);"></div>
									<?php elseif ( isset( $metadata['pyre_type'][0] ) && 'self-hosted-video' == $metadata['pyre_type'][0] ) : ?>
										<div class="mobile_video_image" style="background-image: url(<?php echo Aione_Sanitize::css_asset_url( get_template_directory_uri() . '/assets/images/video_preview.jpg' ); ?>);"></div>
									<?php endif; ?>
...
if( ! function_exists( 'aione_get_page_title_bar_contents' ) ) {
	function aione_get_page_title_bar_contents( $post_id, $get_secondary_content = TRUE ) {

		if ( $get_secondary_content ) {
			ob_start();
			if ( oxo_get_option( 'page_title_bar_bs', 'page_title_breadcrumbs_search_bar', $post_id ) != 'none' ) {
				if ( ( 'Breadcrumbs' == Aione()->settings->get( 'page_title_bar_bs' ) && in_array( get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ), array( 'breadcrumbs', 'default', '' ) ) ) || 'breadcrumbs' == get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ) ) {
					oxo_breadcrumbs();
				} elseif ( ( 'Search Box' == Aione()->settings->get( 'page_title_bar_bs' ) && in_array( get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ), array( 'searchbar', 'default', '' ) ) ) || 'searchbar' == get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ) ) {
					get_search_form();
				}
			}
		...
		if ( '' == get_post_meta( $post_id, 'pyre_page_title_text', true ) || 'default' == get_post_meta( $post_id, 'pyre_page_title_text', true ) ) {
			$page_title_text = Aione()->settings->get( 'page_title_bar_text' );
		} else {
			$page_title_text = get_post_meta( $post_id, 'pyre_page_title_text', true );
		}

		if ( is_search() ) {
			$title = sprintf( __( 'Search results for: %s', 'Aione' ), get_search_query() );
			$subtitle = '';
		}
		...
		if ( ! $title ) {
			...
			if ( is_home() && get_option( 'show_on_front' ) != 'page' ) {
				$title = Aione()->settings->get( 'blog_title' );
			}
			...
			if ( class_exists( 'Tribe__Events__Main' ) && ( ( tribe_is_event() && ! is_single() && ! is_home() ) || is_events_archive() || ( is_events_archive() && is_404() ) ) ) {
				...
				if ( is_day() ) {
					$title = sprintf( __( 'Daily Archives: %s', 'Aione' ), '<span>' . get_the_date() . '</span>' );
				} else if ( is_month() ) {
					$title = sprintf( __( 'Monthly Archives: %s', 'Aione' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'Aione' ) ) . '</span>' );
				} elseif ( is_year() ) {
					$title = sprintf( __( 'Yearly Archives: %s', 'Aione' ), '<span> ' . get_the_date( _x( 'Y', 'yearly archives date format', 'Aione' ) ) . '</span>' );
				} elseif ( is_author() ) {
		...
		if ( ! $subtitle && is_home() && get_option( 'show_on_front' ) != 'page' ) {
			$subtitle = Aione()->settings->get( 'blog_subtitle' );
		}

		if ( ! is_archive() && ! is_search() && ! ( is_home() && ! is_front_page() ) ) {
			if ( 'no' == $page_title_text && ( 'yes' == get_post_meta( $post_id, 'pyre_page_title', true ) || 'yes_without_bar' == get_post_meta( $post_id, 'pyre_page_title', true ) || ( 'hide' != Aione()->settings->get( 'page_title_bar' ) && 'no' != get_post_meta( $post_id, 'pyre_page_title', true ) ) ) ) {
				$title    = '';
				$subtitle = '';
			}
		} else {
			if ( 'hide' != Aione()->settings->get( 'page_title_bar' ) && 'no' == $page_title_text ) {
				$title    = '';
				$subtitle = '';
			}
		}
...
if ( ! function_exists( 'aione_current_page_title_bar' ) ) {
	function aione_current_page_title_bar( $post_id  ) {
		$page_title_bar_contents = aione_get_page_title_bar_contents( $post_id );

		if ( ( ! is_archive() || class_exists( 'WooCommerce' ) && is_shop() ) && 
			 ! is_search() 
		) {
			if ( 'yes' == get_post_meta( $post_id, 'pyre_page_title', true ) || 'yes_without_bar' == get_post_meta( $post_id, 'pyre_page_title', true ) || ( 'hide' != Aione()->settings->get( 'page_title_bar' ) && 'no' != get_post_meta( $post_id, 'pyre_page_title', true ) ) ) {
				if ( is_home() && is_front_page() && ! Aione()->settings->get( 'blog_show_page_title_bar' ) ) {
					// do nothing
				} else {
					if( is_home() && get_post_meta( $post_id, 'pyre_page_title', true ) == 'default' && ! Aione()->settings->get( 'blog_show_page_title_bar' ) ) {
						return;
					}
					aione_page_title_bar( $page_title_bar_contents[0], $page_title_bar_contents[1], $page_title_bar_contents[2] );
				}
			}
		} else {
			if ( is_home() && Aione()->settings->get( 'blog_show_page_title_bar' ) ) {
				aione_page_title_bar( $page_title_bar_contents[0], $page_title_bar_contents[1], $page_title_bar_contents[2] );
			} else {
				if( 'hide' != Aione()->settings->get( 'page_title_bar' ) ) {
					aione_page_title_bar( $page_title_bar_contents[0], $page_title_bar_contents[1], $page_title_bar_contents[2] );
				}
			}
		}
...
if ( ! function_exists( 'aione_featured_images_for_pages' ) ) {
	function aione_featured_images_for_pages() {
		if ( ! post_password_required( get_the_ID() ) ) {

			$html = $video = $featured_images = '';

			if( ! Aione()->settings->get( 'featured_images_pages' ) ) {
				if ( 0 < aione_number_of_featured_images() || get_post_meta( get_the_ID(), 'pyre_video', true ) ) {
					...
					$i = 2;
					while ( $i <= Aione()->settings->get( 'posts_slideshow_number' ) ) :
					...
					$html .= sprintf(
						'<div class="oxo-flexslider flexslider post-slideshow"><ul class="slides">%s%s</ul></div>',
						$video,
						$featured_images
					);
				}
...
if ( ! function_exists( 'aione_featured_images_lightbox' ) ) {
	function aione_featured_images_lightbox( $post_id ) {
		$html = $video = $featured_images = '';
		...
		$html .= sprintf( '<div class="oxo-portfolio-gallery-hidden">%s%s</div>', $video, $featured_images );
...
if ( ! function_exists( 'aione_link_pages' ) ) {
	function aione_link_pages() {
		wp_link_pages( array(
			'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'Aione' ) . '</span>',
			'after'       => '</div>',
...
if ( ! function_exists( 'aione_number_of_featured_images' ) ) {
	function aione_number_of_featured_images() {
		global $post;
		$number_of_images = 0;
		...
		for ( $i = 2; $i <= Aione()->settings->get( 'posts_slideshow_number' ); $i++ ) {
