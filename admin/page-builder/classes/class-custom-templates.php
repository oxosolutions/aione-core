...
if( ! class_exists( 'Oxo_Core_Custom_Templates' ) ) {

	class Oxo_Core_Custom_Templates {
		...
		public function get_custom_templates () {
			...
			$content 		= '<div id="custom_templates_wrapper">
								...
								<h1 class="templates_heading">'.__('Save Template', 'oxo-core').'</h1>
								<div class="save_templates_here">
								<a class="templates_selection" id="oxo_save_custom_tpl" 
								href="JavaScript:void(0)">'.__('Save Layout As Template', 'oxo-core').'</a>
								</div>
								</div>
								<div id="custom_templates_right">
								<h1 class="templates_heading">'.__('Load Template', 'oxo-core').'</h1>';
			//get templates data
			$templates 		= get_option( 'oxo_custom_templates' );
				...
				for( $i = 0; $i < $columns; $i++) {
					...
					foreach( $templates as $key => $value ) {
						...
						$content.= ' <div class="hidden_overlay">
										<a href="JavaScript:void(0)" data-id="'.$key.'"  class="fuiosn_load_template">'.__('LOAD', 'oxo-core').'</a>
										<a href="JavaScript:void(0)" data-id="'.$key.'"  class="oxo_delete_template">'.__('DELETE', 'oxo-core').'</a>
									 </div>';
						$content.= ' <a class="oxo_custom_template templates_selection" href="JavaScript:void(0)">'.$key.'</a>';
						$content.= ' </div>';
		...
		public function get_single_template () {
			...
			$custom_templates = get_option( 'oxo_custom_templates' );
		...
		public function save_single_template () {
			...
			if ( $custom_templates === false ) {
				...
				$response 							= add_option( 'oxo_custom_templates', $custom_templates, '', 'no' );
			} else {
				
				$custom_templates[ stripslashes($_POST['name']) ] = $model;
				
				$response 							= update_option( 'oxo_custom_templates',  $custom_templates );
