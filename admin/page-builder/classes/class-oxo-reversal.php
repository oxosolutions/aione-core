<?php
	 ...
	 * @package   AioneCore
	 * @author    ThemeOxo
	 * @link      http://theme-oxo.com
	 * @copyright OXO Solutions
	 */

	if ( ! class_exists( 'Oxo_Core_Reversal' ) ) {

		class Oxo_Core_Reversal {
			...
			public static $tags = array(
				...
				'oxo_lightbox'          => 'oxo_lightbox',
				'oxo_login'             => 'oxo_login',
				'oxo_register'          => 'oxo_register',
				'oxo_lost_password'     => 'oxo_lost_password',
				...
				//'tooltip'					=> 'tooltip',
				'vimeo'                    => 'vimeo',
				'oxo_widget_area'		=> 'oxo_widget_area',
			...
			ini_set('display_startup_errors',1);*/
				...
				$content = Oxo_Core_Reversal::convert_to_builder_blocks( $content );

				preg_match_all( '/' . Oxo_Core_Reversal::get_shortcode_regex() . '/s', $content, Oxo_Core_Reversal::$matches, PREG_SET_ORDER );

				//$memory_1 = memory_get_usage();


				if ( ! empty( Oxo_Core_Reversal::$matches ) ) {
					foreach ( Oxo_Core_Reversal::$matches as $match ) {
						switch ( $match[2] ) {
							case 'fullwidth':

								$full_width                  = new TF_FullWidthContainer();
								$full_width->config['index'] = $index;
								$full_width->config['id']    = Oxo_Core_Reversal::GUID();
								$index                       = $index + 1;
								$attribs                     = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
								if ( method_exists( 'TF_FullWidthContainer', 'deprecated_args' ) ) {
									$attribs = $full_width->deprecated_args( $attribs );
								}
								$children = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $full_width->config['id'] );
								...
								$full_width = Oxo_Core_Reversal::prepare_full_width( $attribs, $full_width );

								array_push( Oxo_Core_Reversal::$elements, $full_width->element_to_array() );
								break;

							default:
								Oxo_Core_Reversal::convert_builder_elements( $match, $index );
								break;
						}
				...
				//var_dump(Oxo_Core_Reversal::$elements);
				header( "Content-Type: application/json" );
				//echo json_encode( array('count' => count( Oxo_Core_Reversal::$elements ) ) );
				echo json_encode( Oxo_Core_Reversal::$elements );
			...
			public static function get_shortcode_regex( $ignored = false, $all = false ) {
				$shortcode_tags = Oxo_Core_Reversal::$tags;
			...
			public static function GUID() {
				...
				return 'oxob_' . sprintf( '%04X%04X-%04X-%04X-%04X-%04X%04X%04X', mt_rand( 0, 65535 ), mt_rand( 0, 65535 ),
			...
			public static function has_shortcode( $content, $tag, $ignored_shortcode = false ) {
				...
				preg_match_all( '/' . Oxo_Core_Reversal::get_shortcode_regex( $ignored_shortcode ) . '/s', $content, $matches, PREG_SET_ORDER );
				if ( empty( $matches ) ) {
					return false;
				}
			...
			public static function is_shortcode( $content ) {
				foreach ( Oxo_Core_Reversal::$tags as $tag ) {
					if ( Oxo_Core_Reversal::has_shortcode( $content, $tag ) ) {
						return true;
					}
				}
			...
			public static function check_for_child_elements( $content, &$index, $parent = null ) {
				...
				if ( ! empty( $content ) && $content != ' ' ) {
					...
					if ( Oxo_Core_Reversal::is_shortcode( $content ) ) {
						$matches  = null;
						$children = array();
						preg_match_all( '/' . Oxo_Core_Reversal::get_shortcode_regex() . '/s', $content, $matches, PREG_SET_ORDER );

						foreach ( $matches as $match ) {
							$child_id = Oxo_Core_Reversal::convert_builder_elements( $match, $index, $parent );
			...
			public static function create_text_element( $content, &$index, $parent = null ) {
				...
				array_push( Oxo_Core_Reversal::$elements, $text_block->element_to_array() );
			...
			public static function prepare_column_options( $match, $element ) {
				$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match ) );
			...
			public static function convert_builder_elements( $match, &$index, $parent = null ) {
				switch ( $match[2] ) {
					case 'one_full':
						...
						$grid_one->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_one                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_one );
						$index                     = $index + 1;
						$children                  = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_one->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_one->element_to_array() );
					...
					case 'one_half':
						...
						$grid_two->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_two                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_two );
						$index                     = $index + 1;
						$children                  = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_two->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_two->element_to_array() );
					...
					case 'one_third':
						...
						$grid_three->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_three                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_three );
						$index                       = $index + 1;
						$children                    = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_three->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_three->element_to_array() );
					...
					case 'one_fourth':
						...
						$grid_four->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_four                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_four );
						$index                      = $index + 1;
						$children                   = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_four->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_four->element_to_array() );
					...
					case 'one_fifth':
						...
						$grid_five->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_five                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_five );
						$index                      = $index + 1;
						$children                   = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_five->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_five->element_to_array() );
					...
					case 'two_fifth':
						...
						$grid_two_fifth->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_two_fifth                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_two_fifth );
						$index                           = $index + 1;
						$children                        = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_two_fifth->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_two_fifth->element_to_array() );
					...
					case 'three_fifth':
						...
						$grid_three_fifth->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_three_fifth                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_three_fifth );
						$index                             = $index + 1;
						$children                          = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_three_fifth->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_three_fifth->element_to_array() );
					...
					case 'four_fifth':
						...
						$grid_four_fifth->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_four_fifth                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_four_fifth );
						$index                            = $index + 1;
						$children                         = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_four_fifth->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_four_fifth->element_to_array() );
					...
					case 'one_sixth':
						...
						$grid_six->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_six                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_six );
						$index                     = $index + 1;
						$children                  = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_six->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_six->element_to_array() );
					...
					case 'five_sixth':
						...
						$grid_five_six->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_five_six                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_five_six );
						$index                          = $index + 1;
						$children                       = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_five_six->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_five_six->element_to_array() );
					...
					case 'three_fourth':
						...
						$grid_three_fourth->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_three_fourth                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_three_fourth );
						$index                              = $index + 1;
						$children                           = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_three_fourth->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_three_fourth->element_to_array() );
					...
					case 'two_third':
						...
						$grid_two_third->config['id']    = Oxo_Core_Reversal::GUID();
						$grid_two_third                  = Oxo_Core_Reversal::prepare_column_options( $match[3], $grid_two_third );
						$index                           = $index + 1;
						$children                        = Oxo_Core_Reversal::check_for_child_elements( $match[5], $index, $grid_two_third->config['id'] );
						...
						array_push( Oxo_Core_Reversal::$elements, $grid_two_third->element_to_array() );
					...
					case 'alert':
						...
						$alert_box->config['id']    = Oxo_Core_Reversal::GUID();
						$index                      = $index + 1;
						$attribs                    = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']         = stripslashes( $match[5] );
						$alert_box                  = Oxo_Core_Reversal::prepare_alert_box( $attribs, $alert_box );
						if ( ! is_null( $parent ) ) {
							$alert_box->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $alert_box->element_to_array() );
					...
					case 'blog':
						...
						$wp_blog->config['id']    = Oxo_Core_Reversal::GUID();
						$index                    = $index + 1;
						$attribs                  = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$wp_blog                  = Oxo_Core_Reversal::prepare_wp_blog( $attribs, $wp_blog );
						if ( ! is_null( $parent ) ) {
							$wp_blog->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $wp_blog->element_to_array() );
					...
					case 'button':
						...
						$wp_button->config['id']    = Oxo_Core_Reversal::GUID();
						$index                      = $index + 1;
						$attribs                    = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']         = stripslashes( $match[5] );
						$wp_button                  = Oxo_Core_Reversal::prepare_wp_button( $attribs, $wp_button );
						if ( ! is_null( $parent ) ) {
							$wp_button->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $wp_button->element_to_array() );
					...
					case 'checklist':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_checklist_child_attrib( $match, $attribs );
						...
						$checklist->config['id']    = Oxo_Core_Reversal::GUID();
						$index                      = $index + 1;
						$checklist                  = Oxo_Core_Reversal::prepare_checklist( $attribs, $checklist );
						if ( ! is_null( $parent ) ) {
							$checklist->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $checklist->element_to_array() );
					...
					case 'clients':						
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['lightbox'] = 'no';
						$attribs = Oxo_Core_Reversal::get_carousel_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$image_carousel->config['id']    = Oxo_Core_Reversal::GUID();
						$index                           = $index + 1;
						$image_carousel                  = Oxo_Core_Reversal::prepare_carousel( $attribs, $image_carousel );
						if ( ! is_null( $parent ) ) {
							$image_carousel->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $image_carousel->element_to_array() );
					...
					case 'oxo_code':
						...
						$code_block->config['id']    = Oxo_Core_Reversal::GUID();
						$index                       = $index + 1;
						$attribs                     = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						...
						$code_block                  = Oxo_Core_Reversal::prepare_code_block( $attribs, $code_block );
						if ( ! is_null( $parent ) ) {
							$code_block->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $code_block->element_to_array() );
					...
					case 'content_boxes':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_content_boxes_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$content_boxes->config['id']    = Oxo_Core_Reversal::GUID();
						$index                          = $index + 1;
						$content_boxes                  = Oxo_Core_Reversal::prepare_content_boxes( $attribs, $content_boxes );
						if ( ! is_null( $parent ) ) {
							$content_boxes->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $content_boxes->element_to_array() );
					...
					case 'oxo_countdown':
						...
						$countdown->config['id']    = Oxo_Core_Reversal::GUID();
						$index                      = $index + 1;
						$attribs                    = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']         = stripslashes( $match[5] );
						$countdown                  = Oxo_Core_Reversal::prepare_countdown( $attribs, $countdown );
						if ( ! is_null( $parent ) ) {
							$countdown->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $countdown->element_to_array() );
					...
					case 'counters_circle':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_counter_circle_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$counter_circle->config['id']    = Oxo_Core_Reversal::GUID();
						$index                           = $index + 1;
						$counter_circle                  = Oxo_Core_Reversal::prepare_counter_circle( $attribs, $counter_circle );
						if ( ! is_null( $parent ) ) {
							$counter_circle->config['parentId'] = $parent;
						}

						array_push( Oxo_Core_Reversal::$elements, $counter_circle->element_to_array() );
					...
					case 'counters_box':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_counter_box_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$counter_box->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$counter_box                  = Oxo_Core_Reversal::prepare_counter_box( $attribs, $counter_box );
						if ( ! is_null( $parent ) ) {
							$counter_box->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $counter_box->element_to_array() );
					...
					$drop_Cap->config['id'] 			= Oxo_Core_Reversal::GUID();
					$index								= $index + 1;
					$attribs							= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
					$attribs['content']					= stripslashes( $match[5] );
					$drop_Cap							= Oxo_Core_Reversal::prepare_wp_drop_Cap( $attribs, $drop_Cap );
					if ( !is_null( $parent ) ) { $drop_Cap->config['parentId'] = $parent; }
					array_push( Oxo_Core_Reversal::$elements , $drop_Cap->element_to_array() );
				...
				break;*/
					...
					case 'postslider':
						...
						$post_slider->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$post_slider                  = Oxo_Core_Reversal::prepare_post_slider( $attribs, $post_slider );
						if ( ! is_null( $parent ) ) {
							$post_slider->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $post_slider->element_to_array() );
					...
					case 'flip_boxes':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_flip_boxes_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$flip_boxes->config['id']    = Oxo_Core_Reversal::GUID();
						$index                       = $index + 1;
						$flip_boxes                  = Oxo_Core_Reversal::prepare_flip_boxes( $attribs, $flip_boxes );
						if ( ! is_null( $parent ) ) {
							$flip_boxes->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $flip_boxes->element_to_array() );
					...
					case 'fontawesome':
						...
						$font_awesmoe->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$font_awesmoe                  = Oxo_Core_Reversal::prepare_wp_font_awesmoe( $attribs, $font_awesmoe );
						if ( ! is_null( $parent ) ) {
							$font_awesmoe->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $font_awesmoe->element_to_array() );
					...
					case 'map':
						...
						$google_map->config['id']    = Oxo_Core_Reversal::GUID();
						$index                       = $index + 1;
						$attribs                     = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$google_map                  = Oxo_Core_Reversal::prepare_wp_google_map( $attribs, $google_map );
						if ( ! is_null( $parent ) ) {
							$google_map->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $google_map->element_to_array() );
					...
					$high_light->config['id'] 					= Oxo_Core_Reversal::GUID();
					$index										= $index + 1;
					$attribs									= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
					$attribs['content']							= stripslashes( $match[5] );
					$high_light									= Oxo_Core_Reversal::prepare_wp_high_light( $attribs, $high_light );
					
					if ( !is_null( $parent ) ) { $high_light->config['parentId'] = $parent; }
					array_push( Oxo_Core_Reversal::$elements , $high_light->element_to_array() );
				...
				break;*/
					case 'imageframe':
						...
						$image_frame->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						...
						$image_frame = Oxo_Core_Reversal::prepare_image_frame( $attribs, $image_frame );
						if ( ! is_null( $parent ) ) {
							$image_frame->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $image_frame->element_to_array() );
					...
					case 'layerslider':
						...
						$layer_slider->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$layer_slider                  = Oxo_Core_Reversal::prepare_layerslider( $attribs, $layer_slider );
						if ( ! is_null( $parent ) ) {
							$layer_slider->config['parentId'] = $parent;
						}

						array_push( Oxo_Core_Reversal::$elements, $layer_slider->element_to_array() );
					...
					case 'oxo_login':
					case 'oxo_register':
					case 'oxo_lost_password':

						$login                 		= new TF_Login();
						$login->config['index'] 	= $index;
						$login->config['id']    	= Oxo_Core_Reversal::GUID();
						$index                      = $index + 1;
						$attribs                    = array_merge( array( 'oxo_login_type' => $match[2] ), Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) ) );
						$login                  	= Oxo_Core_Reversal::prepare_login( $attribs, $login );
						if ( ! is_null( $parent ) ) {
							$login->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $login->element_to_array() );
					...
					case 'menu_anchor':
						...
						$menu_anchor->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$menu_anchor                  = Oxo_Core_Reversal::prepare_menu_anchor( $attribs, $menu_anchor );
						if ( ! is_null( $parent ) ) {
							$menu_anchor->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $menu_anchor->element_to_array() );
					...
					case 'modal':
						...
						$modal->config['id']    = Oxo_Core_Reversal::GUID();
						$index                  = $index + 1;
						$attribs                = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']     = stripslashes( $match[5] );
						$modal                  = Oxo_Core_Reversal::prepare_modal( $attribs, $modal );
						if ( ! is_null( $parent ) ) {
							$modal->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $modal->element_to_array() );
					...
					$modal_link->config['id'] 					= Oxo_Core_Reversal::GUID();
					$index										= $index + 1;
					$attribs									= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
					$modal_link									= Oxo_Core_Reversal::prepare_modal_link( $attribs, $modal_link );
					if ( !is_null( $parent ) ) { $modal_link->config['parentId'] = $parent; }
					array_push( Oxo_Core_Reversal::$elements , $modal_link->element_to_array() );
				...
				break;*/
					case 'person':
						...
						$person_box->config['id']    = Oxo_Core_Reversal::GUID();
						$index                       = $index + 1;
						$attribs                     = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']          = stripslashes( $match[5] );
						$person_box                  = Oxo_Core_Reversal::prepare_person_box( $attribs, $person_box );
						if ( ! is_null( $parent ) ) {
							$person_box->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $person_box->element_to_array() );
					...
					$popover->config['id'] 						= Oxo_Core_Reversal::GUID();
					$index										= $index + 1;
					$attribs									= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
					$attribs['trigger_content']					= $match[5];
					$popover									= Oxo_Core_Reversal::prepare_popover( $attribs, $popover );
					if ( !is_null( $parent ) ) { $popover->config['parentId'] = $parent; }
					array_push( Oxo_Core_Reversal::$elements , $popover->element_to_array() );
				...
				break;*/
					...
					$pricing_table->config['id'] 				= Oxo_Core_Reversal::GUID();
					$index										= $index + 1;
					$attribs									= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
					$pricing_table								= Oxo_Core_Reversal::prepare_pricing_table( $attribs, $pricing_table );
					if ( !is_null( $pricing_table ) ) { $pricing_table->config['parentId'] = $parent; }
					array_push( Oxo_Core_Reversal::$elements , $pricing_table->element_to_array() );
				...
				break;*/
					case 'progress':
						...
						$progress_bar->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']            = stripslashes( $match[5] );
						$progress_bar                  = Oxo_Core_Reversal::prepare_progress_bar( $attribs, $progress_bar );
						if ( ! is_null( $parent ) ) {
							$progress_bar->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $progress_bar->element_to_array() );
					...
					case 'recent_posts':
						...
						$recent_posts->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$recent_posts                  = Oxo_Core_Reversal::prepare_recent_posts( $attribs, $recent_posts );
						if ( ! is_null( $parent ) ) {
							$recent_posts->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $recent_posts->element_to_array() );
					...
					case 'recent_works':
						...
						$recent_works->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$recent_posts                  = Oxo_Core_Reversal::prepare_recent_works( $attribs, $recent_works );
						if ( ! is_null( $parent ) ) {
							$recent_works->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $recent_works->element_to_array() );
					...
					case 'rev_slider':
						...
						$revolution->config['id']    = Oxo_Core_Reversal::GUID();
						$index                       = $index + 1;
						$attribs                     = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$revolution                  = Oxo_Core_Reversal::prepare_rev_slider( $attribs, $revolution );
						if ( ! is_null( $parent ) ) {
							$revolution->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $revolution->element_to_array() );
					...
					case 'section_separator':
						...
						$section_sep->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$section_sep                  = Oxo_Core_Reversal::prepare_section_separator( $attribs, $section_sep );
						if ( ! is_null( $parent ) ) {
							$section_sep->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $section_sep->element_to_array() );
					...
					case 'separator':
						...
						$separator->config['id']    = Oxo_Core_Reversal::GUID();
						$index                      = $index + 1;
						$attribs                    = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$separator                  = Oxo_Core_Reversal::prepare_separator( $attribs, $separator );
						if ( ! is_null( $parent ) ) {
							$separator->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $separator->element_to_array() );
					...
					case 'sharing':
						...
						$sharing_box->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$sharing_box                  = Oxo_Core_Reversal::prepare_sharing_box( $attribs, $sharing_box );
						if ( ! is_null( $parent ) ) {
							$sharing_box->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $sharing_box->element_to_array() );
					...
					case 'slider':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_slider_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$slider->config['id']    = Oxo_Core_Reversal::GUID();
						$index                   = $index + 1;
						$slider                  = Oxo_Core_Reversal::prepare_slider( $attribs, $slider );
						if ( ! is_null( $parent ) ) {
							$slider->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $slider->element_to_array() );
					...
					case 'soundcloud':
						...
						$sound_cloud->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$sound_cloud                  = Oxo_Core_Reversal::prepare_sound_cloud( $attribs, $sound_cloud );
						if ( ! is_null( $parent ) ) {
							$sound_cloud->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $sound_cloud->element_to_array() );
					...
					case 'social_links':
						...
						$social_links->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$social_links                  = Oxo_Core_Reversal::prepare_social_links( $attribs, $social_links );
						if ( ! is_null( $parent ) ) {
							$social_links->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $social_links->element_to_array() );
					...
					case 'oxo_tabs':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_tabs_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$tabs->config['id']    = Oxo_Core_Reversal::GUID();
						$index                 = $index + 1;
						$tabs                  = Oxo_Core_Reversal::prepare_tabs( $attribs, $tabs );
						if ( ! is_null( $parent ) ) {
							$tabs->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $tabs->element_to_array() );
					...
					case 'tagline_box':
						...
						$tagline_box->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$attribs                      = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']           = stripslashes( $match[5] );
						$tagline_box                  = Oxo_Core_Reversal::prepare_tagline_box( $attribs, $tagline_box );
						if ( ! is_null( $parent ) ) {
							$tagline_box->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $tagline_box->element_to_array() );
					...
					case 'testimonials':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_testimonials_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$testimonial->config['id']    = Oxo_Core_Reversal::GUID();
						$index                        = $index + 1;
						$testimonial                  = Oxo_Core_Reversal::prepare_testimonials( $attribs, $testimonial );
						if ( ! is_null( $parent ) ) {
							$testimonial->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $testimonial->element_to_array() );
					...
					case 'title':
						...
						$title->config['id']    = Oxo_Core_Reversal::GUID();
						$index                  = $index + 1;
						$attribs                = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']     = stripslashes( $match[5] );
						$title                  = Oxo_Core_Reversal::prepare_title( $attribs, $title );
						if ( ! is_null( $parent ) ) {
							$title->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $title->element_to_array() );
					...
					case 'form':
						$form                  = new TF_Form();
						$form ->config['index'] = $index;
						$form ->config['id']    = Oxo_Core_Reversal::GUID();
						$index                  = $index + 1;
						$attribs                = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']     = stripslashes( $match[5] );
						$form                   = Oxo_Core_Reversal::prepare_form( $attribs, $form );
						if ( ! is_null( $parent ) ) {
							$title->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $form ->element_to_array() );

						return array( 'id' => $form ->config['id'] );
						break;
						
					case 'aionegallery':
						$aionegallery                  = new TF_AioneGallery();
						$aionegallery ->config['index'] = $index;
						$aionegallery ->config['id']    = Oxo_Core_Reversal::GUID();
						$index                  = $index + 1;
						$attribs                = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']     = stripslashes( $match[5] );
						$aionegallery           = Oxo_Core_Reversal::prepare_aionegallery( $attribs, $aionegallery );
						if ( ! is_null( $parent ) ) {
							$title->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $aionegallery ->element_to_array() );

						return array( 'id' => $aionegallery ->config['id'] );
						break;
					...
					case 'accordian':
						...
						$attribs = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs = Oxo_Core_Reversal::get_toggles_child_attrib( $match, $attribs );
						foreach ( $attribs['addmore'] as $am_key => $am_value ) {
							foreach ( $am_value as $am_actual_key => $am_actual_value ) {
								if ( $am_actual_value == null ) {
									$attribs['addmore'][ $am_key ][ $am_actual_key ] = '';
						...
						$toggles->config['id']    = Oxo_Core_Reversal::GUID();
						$index                    = $index + 1;
						$toggles                  = Oxo_Core_Reversal::prepare_toggles( $attribs, $toggles );
						if ( ! is_null( $parent ) ) {
							$toggles->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $toggles->element_to_array() );
					...
					$tooltip->config['id'] 				= Oxo_Core_Reversal::GUID();
					$index								= $index + 1;
					$attribs							= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
					$attribs['content']					= stripslashes( $match[5] );
					$tooltip							= Oxo_Core_Reversal::prepare_tooltip( $attribs, $tooltip );
					if ( !is_null( $parent ) ) { $tooltip->config['parentId'] = $parent; }
					array_push( Oxo_Core_Reversal::$elements , $tooltip->element_to_array() );
				...
				break;*/
					case 'vimeo':
						...
						$vimeo->config['id']    = Oxo_Core_Reversal::GUID();
						$index                  = $index + 1;
						$attribs                = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']     = stripslashes( $match[5] );
						$vimeo                  = Oxo_Core_Reversal::prepare_vimeo( $attribs, $vimeo );
						if ( ! is_null( $parent ) ) {
							$vimeo->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $vimeo->element_to_array() );

						return array( 'id' => $vimeo->config['id'] );
						break;
						
					case 'oxo_widget_area':

						$widget_area              		= new TF_WidgetArea();
						$widget_area->config['index'] 	= $index;
						$widget_area->config['id']    	= Oxo_Core_Reversal::GUID();
						$index                      	= $index + 1;
						$attribs                    	= Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']         	= stripslashes( $match[5] );
						$widget_area                  	= Oxo_Core_Reversal::prepare_widget_area( $attribs, $widget_area );
						if ( ! is_null( $parent ) ) {
							$widget_area->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $widget_area->element_to_array() );
					...
					case 'featured_products_slider':
						...
						$featured_woo_slider->config['id']    = Oxo_Core_Reversal::GUID();
						$index                                = $index + 1;
						$attribs                              = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']                   = stripslashes( $match[5] );
						$featured_woo_slider                  = Oxo_Core_Reversal::prepare_featured_products_slider( $attribs, $featured_woo_slider );

						if ( ! is_null( $parent ) ) {
							$featured_woo_slider->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $featured_woo_slider->element_to_array() );
					...
					case 'products_slider':
						...
						$woo_carousel->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']            = stripslashes( $match[5] );
						$woo_carousel                  = Oxo_Core_Reversal::prepare_products_slider( $attribs, $woo_carousel );

						if ( ! is_null( $parent ) ) {
							$woo_carousel->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $woo_carousel->element_to_array() );
					...
					case 'youtube':
						...
						$youtube->config['id']    = Oxo_Core_Reversal::GUID();
						$index                    = $index + 1;
						$attribs                  = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']       = stripslashes( $match[5] );
						$youtube                  = Oxo_Core_Reversal::prepare_youtube( $attribs, $youtube );
						if ( ! is_null( $parent ) ) {
							$youtube->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $youtube->element_to_array() );
					...
					case 'text':
						...
						break;
					case 'oxoslider':
						$oxoslider                  = new TF_OxoSlider();
						$oxoslider->config['index'] = $index;
						$oxoslider->config['id']    = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']            = stripslashes( $match[5] );
						$oxoslider                  = Oxo_Core_Reversal::prepare_oxoslider( $attribs, $oxoslider );

						if ( ! is_null( $parent ) ) {
							$oxoslider->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $oxoslider->element_to_array() );

						return array( 'id' => $oxoslider->config['id'] );
						break;
					case 'oxo_events':
						$events                  	   = new TF_OxoEvents();
						$events->config['index'] 	   = $index;
						$events->config['id']    	   = Oxo_Core_Reversal::GUID();
						$index                         = $index + 1;
						$attribs                       = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						$attribs['content']            = stripslashes( $match[5] );
						$events                  	   = Oxo_Core_Reversal::prepare_events( $attribs, $events );

						if ( ! is_null( $parent ) ) {
							$events->config['parentId'] = $parent;
						}
						array_push( Oxo_Core_Reversal::$elements, $events->element_to_array() );
			...
			public static function get_checklist_child_attrib( $match, $attribs ) {
				...
				preg_match_all( '/' . Oxo_Core_Reversal::get_shortcode_regex() . '/s', $match[5], $matches, PREG_SET_ORDER );

				if ( is_array( $matches ) && count( $matches ) > 0 ) {
					foreach ( $matches as $match ) {

						$child_attr         = Oxo_Core_Reversal::shortcode_parse_atts( stripslashes( $match[3] ) );
						if ( isset( $child_attr['icon'] ) ) {
							$child_attr['icon'] = OxoCore_Plugin::font_awesome_name_handler( $child_attr['icon'] );
			...
			public static function prepare_alert_box( $attribs, $element ) {
				...
				foreach ( $attribs as $key => $value ) {
					...
					switch ( $key ) {
						...
						case 'icon';
							$element->config['subElements'][4]['value'] = OxoCore_Plugin::font_awesome_name_handler( $attribs[ $key ] );
							break;
			...
			public static function prepare_wp_button( $attribs, $element ) {
				...
				foreach ( $attribs as $key => $value ) {
					switch ( $key ) {
						...
						case 'icon';
							$element->config['subElements'][17]['value'] = OxoCore_Plugin::font_awesome_name_handler( $attribs[ $key ] );
							break;
			...
			public static function prepare_checklist( $attribs, $element ) {
				...
				foreach ( $attribs as $key => $value ) {
					switch ( $key ) {
						...
						case 'icon':
							$element->config['subElements'][0]['value'] = OxoCore_Plugin::font_awesome_name_handler( $attribs[ $key ] );
							break;
			...
			public static function prepare_section_separator( $attribs, $element ) {
				...
				foreach ( $attribs as $key => $value ) {
					...
					switch ( $key ) {
						...
						case 'icon':
							$element->config['subElements'][1]['value'] = OxoCore_Plugin::font_awesome_name_handler( $attribs[ $key ] );
							break;
			...
			public static function prepare_separator( $attribs, $element ) {
				...
				foreach ( $attribs as $key => $value ) {
					...
					switch ( $key ) {
						...
						case 'icon':
							$element->config['subElements'][5]['value'] = OxoCore_Plugin::font_awesome_name_handler( $attribs[ $key ] );
							break;
			 ...
			 * @return Array Element
			 */
			public static function prepare_form( $attribs, $element ) {
			 ...
			 * @return Array Element
			 */
			public static function prepare_aionegallery( $attribs, $element ) {
			 ...
			 * @return Array Element
			 */
			public static function prepare_oxoslider( $attribs, $element ) {
			...
			public static function prepare_builder_blocks( $matches ) {
				if ( in_array( $matches[2], Oxo_Core_Reversal::$tags ) ) {
					Oxo_Core_Reversal::$prepared_builder_blocks[ Oxo_Core_Reversal::$builder_blocks_count ] = array_merge( array(), $matches ); //for backward compatibility
					$shortcode                                                                                    = '[oxo:' . Oxo_Core_Reversal::$builder_blocks_count . ']';
					Oxo_Core_Reversal::$builder_blocks_count ++;
			...
			public static function convert_to_builder_blocks( $content ) {
				$content = preg_replace_callback( '/' . get_shortcode_regex() . '/s', 'Oxo_Core_Reversal::prepare_builder_blocks', $content );

				$split_content = preg_split( '/(\[oxo:\d+\])/s', $content, - 1, PREG_SPLIT_DELIM_CAPTURE );

				$buffer = '';

				foreach ( $split_content as $matched_content ) {
					if ( preg_match_all( '/\[oxo:(\d+)\]/s', $matched_content, $matches ) ) {
