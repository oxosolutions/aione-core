...
if( ! class_exists( 'Oxo_Core_Shortcodes_Parser' ) ) {

	class Oxo_Core_Shortcodes_Parser {
		...
		public static function check_builder_elements ( $content ) {
			...
			if ( $post ) {
				self::$meta_content = get_post_meta( $post->ID, "oxo_builder_content" , TRUE); //get builder contents 
				if( ! empty( self::$meta_content ) ) { //if page built through oxo builder
					$short_codes_content = Oxo_Core_Shortcodes_Parser::parse_column_options( );  // let the magic begin
			...
			foreach ( self::$meta_content as $element ) { //traverse elements
				...
				switch ($css_class) { //switch on unique element
					...
					case 'grid_two' :
						...
						$short_codes.= '[one_half '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						//check if child elements exist. Then parse elements one by one
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_three' :
						...
						$short_codes.= '[one_third '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_four' :
						...
						$short_codes.= '[one_fourth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_five' :
						...
						$short_codes.= '[one_fifth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_two_fifth' :
						...
						$short_codes.= '[two_fifth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_three_fifth' :
						...
						$short_codes.= '[three_fifth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_four_fifth' :
						...
						$short_codes.= '[four_fifth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_six' :
						...
						$short_codes.= '[one_sixth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_five_sixth' :
						...
						$short_codes.= '[five_sixth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_three_fourth' :
						...
						$short_codes.= '[three_fourth '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_two_third' :
						...
						$short_codes.= '[two_third '.Oxo_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'oxo_full_width' :
						$short_codes.= Oxo_Core_Shortcodes_Parser::build_full_width_container_shortocde( $element['subElements'] );
						$short_codes.= Oxo_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					default: //default case. For elements without layout column
						if ( !isset ( $element['parentId'] ) ) { //if element does not have any parent (column element)
							...
							$short_codes.= Oxo_Core_Shortcodes_Parser::parse_builder_elements( $element );
						}
				}
			}
		...
		private static function parse_column_element ( $element ) {
			...
			if ( $child_elements > 0 ) {
				$colum_elements = Oxo_Core_Shortcodes_Parser::parse_child_elements( $element );
			}
		...
		private static function parse_child_elements ($element ) {
			...
			foreach ($element['childrenId'] as $child) {
				...
				$builder_element = Oxo_Core_Shortcodes_Parser::search_child_element ( $child['id'] );
				
				if ($builder_element != false) {
					
					$builder_element_shortcode.= Oxo_Core_Shortcodes_Parser::parse_builder_elements($builder_element);
				}
		...
		private static function parse_builder_elements ( $element ) {
			...
			switch ($element['php_class']) { //switch on unique element
				...
				case 'TF_AlertBox':
					return Oxo_Core_Shortcodes_Parser::build_alert_shortocde( $element['subElements'] );
				break;
				
				case 'TF_WpBlog':
					return Oxo_Core_Shortcodes_Parser::build_blog_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_ButtonBlock':
					return Oxo_Core_Shortcodes_Parser::build_button_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_CheckList' :
					return Oxo_Core_Shortcodes_Parser::build_checklist_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_ClientSlider' : 
					return Oxo_Core_Shortcodes_Parser::build_client_slider_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_ContentBoxes' : 
					return Oxo_Core_Shortcodes_Parser::build_content_box_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_CounterCircle' : 
					return Oxo_Core_Shortcodes_Parser::build_counter_circle_shortocde( $element['subElements'] ) ;
				break; 
				
				case 'TF_CounterBox' :
					return Oxo_Core_Shortcodes_Parser::build_counter_box_shortocde( $element['subElements'] ) ;
				break;
				
				/*case 'TF_DropCap' :
					return Oxo_Core_Shortcodes_Parser::build_dropcap_shortocde( $element['subElements'] ) ;
				break;*/
				
				case 'TF_PostSlider' :
					return Oxo_Core_Shortcodes_Parser::build_post_slider_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_FlipBoxes' :
					return Oxo_Core_Shortcodes_Parser::build_flip_boxes_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_FontAwesome' :
					return Oxo_Core_Shortcodes_Parser::build_font_awesome_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_FullWidthContainer' :
					return Oxo_Core_Shortcodes_Parser::build_full_width_container_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_GoogleMap' :
					return Oxo_Core_Shortcodes_Parser::build_google_map_shortocde( $element['subElements'] ) ; 
				break;
				
				/*case 'TF_HighLight' :
					return Oxo_Core_Shortcodes_Parser::build_highlight_shortocde( $element['subElements'] ) ; 
				break;*/
				
				case 'TF_ImageFrame' :
					return Oxo_Core_Shortcodes_Parser::build_image_frame_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_ImageCarousel' :
					return Oxo_Core_Shortcodes_Parser::build_image_carousel_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_LightBox' :
					return Oxo_Core_Shortcodes_Parser::build_light_box_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_LayerSlider' :
					return Oxo_Core_Shortcodes_Parser::build_layer_slider_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_MenuAnchor' :
					return Oxo_Core_Shortcodes_Parser::build_menu_anchor_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Modal' :
					return Oxo_Core_Shortcodes_Parser::build_modal_shortocde( $element['subElements'] ) ; 
				break;
				
				/*case 'TF_Modal_Link' :
					return Oxo_Core_Shortcodes_Parser::build_modal_link_shortocde( $element['subElements'] ) ; 
				break;*/
				
				case 'TF_Person' :
					return Oxo_Core_Shortcodes_Parser::build_person_shortocde( $element['subElements'] ) ; 
				break;
				
				/*case 'TF_Popover' :
					return Oxo_Core_Shortcodes_Parser::build_popover_shortocde( $element['subElements'] ) ; 
				break;*/
				
				case 'TF_PricingTable' :
					return Oxo_Core_Shortcodes_Parser::build_pricing_table_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_ProgressBar' :
					return Oxo_Core_Shortcodes_Parser::build_progress_bar_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_RecentPosts' :
					return Oxo_Core_Shortcodes_Parser::build_recent_posts_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_RecentWorks' : 
					return Oxo_Core_Shortcodes_Parser::build_recent_works_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_RevolutionSlider' :
					return Oxo_Core_Shortcodes_Parser::build_rev_slider_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_SectionSeparator' :
					return Oxo_Core_Shortcodes_Parser::build_section_separator_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Separator' :
					return Oxo_Core_Shortcodes_Parser::build_separator_shortocde( $element['subElements'] ) ; 
				break; 
				
				case 'TF_SharingBox' :
					return Oxo_Core_Shortcodes_Parser::build_sharing_box_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Slider' :
					return Oxo_Core_Shortcodes_Parser::build_slider_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_SoundCloud' :
					return Oxo_Core_Shortcodes_Parser::build_soundcloud_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_SocialLinks' :
					return Oxo_Core_Shortcodes_Parser::build_social_links_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Tabs' :
					return Oxo_Core_Shortcodes_Parser::build_tabs_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Table' :
					return Oxo_Core_Shortcodes_Parser::build_table_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_TaglineBox' :
					return Oxo_Core_Shortcodes_Parser::build_tagline_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Testimonial' :
					return Oxo_Core_Shortcodes_Parser::build_testimonial_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_TextBlock' :
					return Oxo_Core_Shortcodes_Parser::build_text_block_shortocde( $element['subElements'] ) ; 
				break;
				
				case 'TF_Title':
					return Oxo_Core_Shortcodes_Parser::build_title_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_Toggles' :
					return Oxo_Core_Shortcodes_Parser::build_toggles_shortocde( $element['subElements'] ) ;
				break;
				
				/*case 'TF_Tooltip':
					return Oxo_Core_Shortcodes_Parser::build_tooltip_shortocde( $element['subElements'] ) ;
				break;*/
				
				case 'TF_Vimeo':
					return Oxo_Core_Shortcodes_Parser::build_vimeo_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_WooFeatured' :
					return Oxo_Core_Shortcodes_Parser::build_woo_featured_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_WooCarousel' :
					return Oxo_Core_Shortcodes_Parser::build_woo_carousel_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_WooShortcodes' :
					return Oxo_Core_Shortcodes_Parser::build_woo_shortcodes( $element['subElements'] ) ;
				break;

				case 'TF_Youtube':
					return Oxo_Core_Shortcodes_Parser::build_youtube_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_Form':
					return Oxo_Core_Shortcodes_Parser::build_form_shortocde( $element['subElements'] ) ;
				break;
				
				case 'TF_AioneGallery':
					return Oxo_Core_Shortcodes_Parser::build_aionegallery_shortocde( $element['subElements'] ) ;
				break;
		...
		**/
		 ...
		 * @return 		String		Youtube doable shortcode
		 */
		private static function build_form_shortocde ( $args ) {
		
			$shortcode_data = ' [gravityform ';
			$shortcode_data.= ' id="'.$args[0]['value'].'"';
			$shortcode_data.= ' class="'.$args[1]['value'].'"]';
			return $shortcode_data;
		 ...
		 * @return 		String		Youtube doable shortcode
		 */
		private static function build_aionegallery_shortocde ( $args ) {
		
			$shortcode_data = ' [gallery ';
			$shortcode_data.= ' class="'.$args[0]['value'].'"';
			$shortcode_data.= ' id="'.$args[1]['value'].'"]';
			return $shortcode_data;
