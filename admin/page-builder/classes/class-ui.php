...
if( ! class_exists( 'Oxo_Core_PageBuilder_UI' ) ) {

	class Oxo_Core_PageBuilder_UI {
		...
		public function wrap_default_editor() {
			...
			if ( in_array( $screen->id, $allowed_screens) ) {
				...
				$pagebuilder_label 	= __( 'Show Builder', 'oxo-core' );
				$default_label  	= __( 'Hide Builder', 'oxo-core' );
				$status		 	= get_post_meta($post_ID, '_oxoPageBuilderStatus', true);
				echo '<a id="oxo-pb-switch-button" href="javascript:void(0);" class="button button-icon-padding button-icon-color dashicons-before dashicons-plus" data-active-button="' . $default_label . '" data-inactive-button="' . $pagebuilder_label . '">' . $pagebuilder_label . '</a>';
		...
		public function close_default_editor_wrap() {
			...
			if ( in_array( $screen->id, $allowed_screens) ) {
				...
				echo '<textarea class="oxo-builder-pagecontent">' . $content . '</textarea>';
