...
( function($) {
	...
	DdElementParser.generateHtml = function (model, settings_lvl) {
        ...
        if( model.get('subElements')[0].id == 'oxo_settings_lvl' ) {
        	lvl_exists = true;
		...
		jQuery.each ( model.get('subElements') ,function( key, element ){
			...
			if( element['type'] == ADDMORE ) {
				...
				for (var i = 0; i < elementsCount; i++) {
					...
					output 		+= "<tr class='child-clone-row'><td><a href='#' class='oxo-expand-child'>" + model.get('name') + " Item " + (i + 1) + "<i class='oxoa-plus2'></i></a><div class='child-options' style='display: none;'>";
					...
					output 		+="<a class='child-clone-row-remove oxo-shortcodes-button' href='JavaScript:void(0)'>Remove</a>";
					output 		+= "</div></td></tr>";
				}

				output 			+= "<tr><td><a id='oxo-child-add' href='JavaScript:void(0)'>" + element['buttonText'] + "</a></td></tr>";
				output 			+= "</table>";

			} else {
				if ( model.get('subElements')[0].value != 'oxo_login' && ( element['id'] == 'oxo_register' || element['id'] == 'oxo_lost_password' ) ) {
					return;
				}
        ...
        if ($hasGroups) {
            output = $orig_output;
            var $tabID = 1;
            $menu = '<ul class="oxo-tabs-menu">';
            var $tabs = '<div class="oxo-tabs">';

            if ($group_html['Default'] != "") {
                $menu += '<li class="current"><a href="#tab-'+$tabID+'">Default</a></li>';
                $tabs += '<div id="tab-'+$tabID+'" class="oxo-tab-content" style="display:block;">'+$group_html['Default']+'</div>';
                $tabID++;
            }
            ...
            for ( var key in $group_html ) {
                if (key != "Default") {
                    ...
                    $tabs += '<div id="tab-'+$tabID+'" class="oxo-tab-content"'+$class+'>'+$group_html[key]+'</div>';
                    $tabID++;
                }
            }
            $menu += "</ul>";
	...
	DdElementParser.parseElementType = function (element) {
		...
		switch (element['type']) {
			...
			case COLOR:
				...
				output 		+= '<input type="text" class="text-field oxo-color-field" value="' + nl2br(element['value'],false) + '" id="' + element['id'] + '" name="' + element['id'] + '" size="50"/>';
			break;

			case GALLERY:
				output 		+= '<a href="'+element['id']+'" class="oxo-gallery-button oxo-shortcodes-button">Attach Images to Gallery</a>';
			break;
			...
			case UPLOAD:
				...
				output 		+= '<input type="hidden" class="oxo-form-text oxo-form-upload oxo-input" name="'+element['id'] +'" id="'+element['id'] +'" value="'+element['value'] +'" />' + "\n";
				output 		+= '<a href="'+element['id'] +'" class="oxob-upload-button '+button_class+'" data-upid="'+element['upid'] +'">'+button_text+'</a>';
				output		+= '<a href="'+element['id'] +'" style="'+edit_style+'" class="oxob-edit-button" data-upid="'+element['upid'] +'">Edit</a>';
				output 		+= '</div>';
			...
			break;
			case HTML_EDITOR:
				// wpautop is required https://github.com/Theme-Oxo/Aione/issues/2222
