<?php
/**
 * OxoHelper class with static methods
 */
class OxoHelper  {
	/**
	...
	public static function OBJECT_TO_ARRAY( $object ) {
		...
		if ( is_array($object) ) {
			...
			*/
			return array_map("OxoHelper::OBJECT_TO_ARRAY", $object);
		}
		else {
				// Return array
				return $object;
	 ...
	 * @return: data array
	 */
	public static function oxo_shortcodes_categories ( $taxonomy, $empty_choice = false, $empty_choice_label = 'Default' ) {
	 ...
	 * @return: data array
	 */
	 public static function oxo_create_dropdown_data ( $start, $end, $dataArray = array() ) {
	...
	public static function GET_ICONS_LIST() {
		...
		$icons = new FAIterator(OXO_BUILDER_FA_PATH);
		$iconssArray = array();
		$output = '';
	...
	public static function get_layerslider_slides () {
		global $wpdb;
		$slides_array['oxo_0'] 	= 'Select a slider';
		// Table name
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
			...
			if(isset($slides) && $slides){
				foreach($slides as $key => $val){
					$slides_array['oxo_'.$key] = 'LayerSlider #'.($key);
				}
			}
		}
	...
	public static function get_revslider_slides () {
		...
		global $wpdb;
		$revsliders['oxo_0'] 		= 'Select a slider';
			...
			if(function_exists('rev_slider_shortcode')) {
				...
				if($get_sliders) {
					foreach($get_sliders as $slider) {
						$revsliders['oxo_'.$slider->alias] = $slider->title;
					}
				}
			}
		}
	...
	/**
	 * Returns array of theme oxo slider slide groups
	 * @param NULL
	 * @return array slide keys array
	 */
