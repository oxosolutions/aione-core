...
abstract class DDElementTemplate {
		...
		foreach ( $this->config['subElements'] as $key => $element ) {
			...
			if( $element['type'] == ElementTypeEnum::ADDMORE ) {
				...
				for ($i = 0; $i < $elementsCount; $i++) {
					...
					$output 	.="<a class='child-clone-row-remove oxo-shortcodes-button' 
									href='JavaScript:void(0)'>".__('Remove', 'oxo-core')."</a>";
					$output 	.= "</td></tr>";
				}
				$output 		.= "<tr><td><a id='oxo-child-add' href='JavaScript:void(0)'>".$element['buttonText']."</a>
									</td></tr>";
				$output 		.= "</table>";
	...
	private function parseElementType( $element ) {
		...
		switch ($element['type']) {
			...
			case ElementTypeEnum::COLOR:
				$output .= '<input type="text" class="text-field oxo-color-field" value="'.nl2br($element['value']).'" id="'.$element['id'].'" name="'.$element['id'].'" size="50"/>';
			break;
			
			case ElementTypeEnum::GALLERY:
				$output .= '<a href="'.$element['id'].'" class="oxo-gallery-button oxo-shortcodes-button">'.__('Attach Images to Gallery', 'oxo-core').'</a>';
			break;
			
			case ElementTypeEnum::UPLOAD:
				$button_class = ( $element['value'] == 'oxo-hidden-img' ? '' 		: 'remove-image' ); 
				$button_text  = ( $element['value'] == 'oxo-hidden-img' ? __('Upload', 'oxo-core') 	: __('Remove', 'oxo-core') ); 
				$output .= '<div class="oxo-upload-container">';
				$output .= '<img src="' . $element['value'] . '" alt="Image" class="uploaded-image" />';
				$output .= '<input type="hidden" class="oxo-form-text oxo-form-upload oxo-input" 
							name="' . $element['id'] . '" id="' . $element['id'] . '" value="' . $element['value'] . '" />' . "\n";
				$output .= '<a href="' . $element['id'] . '" class="oxo-upload-button '.$button_class.'" 
			...
			case ElementTypeEnum::ICON_BOX:
				$iconsArray = OxoHelper::GET_ICONS_LIST();
	...
	*/
	public function create_visual_editor( $params ) {
		...
		$innerHtml.= "<div class='oxo-element-label'>".$this->config['name']."</div>";
