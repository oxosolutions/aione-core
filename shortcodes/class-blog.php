...
class OxoSC_Blog {
	...
	public function __construct() {
		...
		// containers
		add_action( 'oxo_blog_shortcode_before_loop', array( $this, 'before_loop' ) );
		add_action( 'oxo_blog_shortcode_before_loop_timeline', array( $this, 'before_loop_timeline' ) );
		add_action( 'oxo_blog_shortcode_after_loop', array( $this, 'after_loop' ) );

		// post / loop basic structure
		add_action( 'oxo_blog_shortcode_loop_header', array( $this, 'loop_header' ) );
		add_action( 'oxo_blog_shortcode_loop_footer', array( $this, 'loop_footer' ) );
		add_action( 'oxo_blog_shortcode_loop_content', array( $this, 'loop_content' ) );
		add_action( 'oxo_blog_shortcode_loop_content', array( $this, 'page_links' ) );
		add_action( 'oxo_blog_shortcode_loop', array( $this, 'loop' ) );

		// special blog layout structure
		add_action( 'oxo_blog_shortcode_wrap_loop_open', array( $this, 'wrap_loop_open' ) );
		add_action( 'oxo_blog_shortcode_wrap_loop_close', array( $this, 'wrap_loop_close' ) );
		add_action( 'oxo_blog_shortcode_date_and_format', array( $this, 'add_date_box' ) );
		add_action( 'oxo_blog_shortcode_date_and_format', array( $this, 'add_format_box' ) );
		add_action( 'oxo_blog_shortcode_timeline_date', array( $this, 'timeline_date' ) );

		// element attributes
		add_filter( 'oxo_attr_blog-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_blog-shortcode-posts-container', array( $this, 'posts_container_attr' ) );
		add_filter( 'oxo_attr_blog-shortcode-loop', array( $this, 'loop_attr' ) );
		add_filter( 'oxo_attr_blog-shortcode-post-title', array( $this, 'post_title_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$oxo_query = new WP_Query( $defaults );
		
		$this->query = $oxo_query;


		//prepare needed wrapping containers
		$html = '';

		$html .= sprintf( '<div %s>', OxoCore_Plugin::attributes( 'blog-shortcode' ) );
		
		if ( self::$args['layout'] == 'grid' &&
			 self::$args['blog_grid_column_spacing'] 
		) {
			$html .= sprintf( '<style type="text/css">.oxo-blog-shortcode-%s .oxo-blog-layout-grid .oxo-post-grid{padding:%spx;}.oxo-blog-shortcode-%s .oxo-posts-container{margin-left: -%spx !important; margin-right:-%spx !important;}</style>', $this->blog_sc_counter, $defaults['blog_grid_column_spacing'] / 2, $this->blog_sc_counter, $defaults['blog_grid_column_spacing'] / 2, $defaults['blog_grid_column_spacing'] / 2 );
		}

		$html .= sprintf( '<div %s>', OxoCore_Plugin::attributes( 'blog-shortcode-posts-container' ) );

		ob_start();
		do_action( 'oxo_blog_shortcode_wrap_loop_open' );
		...
		//do the loop
		if ( $oxo_query->have_posts() ) : while ( $oxo_query->have_posts() ) : $oxo_query->the_post();
			...
			if ( self::$args['layout'] == 'timeline' ) {
				...
				ob_start();
				do_action( 'oxo_blog_shortcode_timeline_date', $date_params );
			...
			ob_start();
			do_action( 'oxo_blog_shortcode_before_loop' );
			...
			if ( self::$args['layout'] == 'grid' || 
				...
				$html .= sprintf( '<div %s>', OxoCore_Plugin::attributes( 'oxo-post-wrapper' ) );
			}
			...
			ob_start();
			do_action( 'oxo_blog_shortcode_loop_header' );

			do_action( 'oxo_blog_shortcode_loop_content' );

			do_action( 'oxo_blog_shortcode_loop_footer' );
			
			do_action( 'oxo_blog_shortcode_after_loop' );
		...
		ob_start();
		do_action( 'oxo_blog_shortcode_wrap_loop_close' );
		...
		if ( self::$args['paging'] ) {
			ob_start();
			oxo_pagination( $this->query->max_num_pages, $range = 2, $this->query );
		...
		if ( self::$args['load_more'] && self::$args['posts_per_page'] != -1 ) {
			$html .= sprintf( '<div class="oxo-load-more-button oxo-clearfix">%s</div>', apply_filters( 'aione_load_more_posts_name', __( 'Load More Posts', 'oxo-core' ) ) );
		}		
	...
	function attr() {
		...
		$blog_layout = '';
		if( self::$args['layout'] == 'timeline' ) {
			$blog_layout = 'oxo-blog-layout-timeline-wrapper';			
		} elseif ( self::$args['layout'] == 'grid' ) {
			$blog_layout = 'oxo-blog-layout-grid-wrapper';	
		} else {
			$blog_layout = sprintf( 'oxo-blog-layout-%s', self::$args['layout'] );
		}

		$attr['class'] = sprintf( 'oxo-blog-shortcode oxo-blog-shortcode-%s oxo-blog-archive %s oxo-blog-%s', $this->blog_sc_counter, $blog_layout, self::$args['scrolling'] );

		if ( ! self::$args['thumbnail'] ) {
			$attr['class'] .= ' oxo-blog-no-images';
		}
	...
	function posts_container_attr() {
		...
		if ( self::$args['load_more'] ) {
			$load_more = ' oxo-posts-container-load-more';
		}
		
		$attr['class'] = sprintf( 'oxo-posts-container oxo-posts-container-%s%s', self::$args['scrolling'], $load_more );		
		
		$attr['data-pages'] = $this->query->max_num_pages;
		
		if ( self::$args['layout'] == 'grid' ) {
			 $attr['class'] .= sprintf( ' oxo-blog-layout-grid oxo-blog-layout-grid-%s isotope', self::$args['blog_grid_columns'] );
	...
	function wrap_loop_open() {
		...
		if ( self::$args['layout'] == 'timeline' ) {
			...
			$wrapper = sprintf( '<div %s><i %s></i></div>', OxoCore_Plugin::attributes( 'oxo-timeline-icon' . $class_timeline_icon ), OxoCore_Plugin::attributes( 'oxo-icon-bubbles' ) );
			$wrapper .= sprintf( '<div %s>', OxoCore_Plugin::attributes( 'oxo-blog-layout-timeline oxo-clearfix' ) );
			$wrapper .= '<div class="oxo-timeline-line"></div>';
		}
	...
	function wrap_loop_close() {
		...
		if ( self::$args['layout'] == 'grid' ) {
			$wrapper .= '<div class="oxo-clearfix"></div>';
		}
	...
	function before_loop() {
		...
		echo sprintf( '<div %s>', OxoCore_Plugin::attributes( 'blog-shortcode-loop' ) ) . "\n";
	...
	function loop_attr() {
		...
		// In any other case, add the correct post class
			...
			$extra_classes[] = sprintf( 'oxo-post-%s', self::$args['layout'] );
		}
		...
		if ( self::$args['layout'] == 'timeline' ) {
			...
			if ( ( $this->post_count % 2 ) > 0 ) {
				$timeline_align = ' oxo-left-column';
			} else {
				$timeline_align = ' oxo-right-column';
			}

			$extra_classes[] = 'oxo-clearfix' . $timeline_align;
		}
	...
	function get_slideshow() {
		...
		if ( ! post_password_required( get_the_ID() ) && function_exists( 'aione_plugins_loaded' ) ) {
	...
	function loop_header() {
		...
		if ( self::$args['layout'] == 'medium-alternate' || 
			...
			$pre_title_content .= sprintf( '<div %s>', OxoCore_Plugin::attributes( 'oxo-date-and-formats' ) );
			ob_start();
			do_action( 'oxo_blog_shortcode_date_and_format' );
			...
			if ( self::$args['meta_all'] ) {
				$meta_data .= aione_render_post_metadata( 'alternate', $this->meta_info_settings );
			}
		}
		...
		if ( self::$args['layout'] == 'grid' ||
			...
			if ( self::$args['meta_info_combined'] > 0 &&
				...
				$content_sep = sprintf( '<div %s></div>', OxoCore_Plugin::attributes('oxo-content-sep' ) );
			}
			...
			if ( self::$args['meta_all'] ) {
				$meta_data .= aione_render_post_metadata( 'grid_timeline', $this->meta_info_settings );
			}
			
			$pre_title_content .=  sprintf( '<div %s%s>', OxoCore_Plugin::attributes( 'oxo-post-content-wrapper' ), $content_wrapper_styles );
		}
		
		$pre_title_content .=  sprintf( '<div %s>', OxoCore_Plugin::attributes( 'oxo-post-content post-content' ) );
		...
		if ( self::$args['layout'] == 'timeline' ) {
			$pre_title_content .= sprintf( '<div %s></div><div %s></div>', OxoCore_Plugin::attributes( 'oxo-timeline-circle' ), OxoCore_Plugin::attributes( 'oxo-timeline-arrow' ) );
		}

		$html = sprintf( '%s<h2 %s>%s</h2>%s%s', $pre_title_content, OxoCore_Plugin::attributes( 'blog-shortcode-post-title' ), $link, $meta_data, $content_sep );
	...
	function loop_footer() {
		...
		if ( self::$args['layout'] == 'grid' || 
			...
			if ( self::$args['meta_info_combined'] > 0 ) {
				...
				echo sprintf( '<div class="oxo-meta-info">%s</div>', $inner_content );
			}
		}
		...
		if ( self::$args['meta_info_combined'] > 0 && self::$args['layout'] == 'large' ||
			...
			echo sprintf( '<div class="oxo-meta-info">%s%s</div>', aione_render_post_metadata( 'standard', $this->meta_info_settings ), $this->read_more() );
		}		
	...
	function add_date_box() {
		...
		$inner_content = sprintf( '<div %s>', OxoCore_Plugin::attributes( 'oxo-date-box updated' ) );

		$inner_content .= sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'oxo-date' ), get_the_time( $smof_data['alternate_date_format_day'] ) );
		$inner_content .= sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'oxo-month-year' ), get_the_time( $smof_data['alternate_date_format_month_year'] ) );
	...
	function add_format_box() {
		...
		$inner_content = sprintf( '<div %s>
								  	<i %s></i>
								  </div>', OxoCore_Plugin::attributes( 'oxo-format-box' ), OxoCore_Plugin::attributes( 'oxo-icon-' . $format_class ) );
	...
	function timeline_date( $date_params ) {
		...
		if ( $args['prev_post_month'] != $args['post_month'] ||
			...
			$inner_content .= sprintf( '<h3 %s>%s</h3>', OxoCore_Plugin::attributes( 'oxo-timeline-date' ), get_the_date( $smof_data['timeline_date_format'] ) );
			$inner_content .= '<div class="oxo-collapse-month">';
		}
	...
	function grid_timeline_comments() {
		...
		if ( self::$args['meta_comments'] ) {
			...
			$comments_icon = sprintf( '<i %s></i>&nbsp;', OxoCore_Plugin::attributes( 'oxo-icon-bubbles' ) );
			
			if ( ! post_password_required( get_the_ID() ) ) {		
				ob_start();
				comments_popup_link( $comments_icon . __( '0', 'oxo-core' ), $comments_icon . __( '1', 'oxo-core' ), $comments_icon . __( '%', 'oxo-core' ) );
				$comments = ob_get_contents();
				ob_get_clean();
			} else {
				$comments = sprintf( '<i class="oxo-icon-bubbles"></i>&nbsp;%s', __( 'Protected', 'oxo-core' ) );					
			}
			
			$inner_content = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'oxo-alignright' ), $comments );
		...
		if ( self::$args['meta_link'] ) {
			...
			if ( self::$args['meta_read'] ) {
				...
				if ( self::$args['layout'] == 'grid' || 
					...
					$read_more_wrapper_class = 'oxo-alignleft';
				} else {
					$read_more_wrapper_class = 'oxo-alignright';
				}
				...
				$inner_content .= sprintf( '<div %s><a class="oxo-read-more" href="%s"%s>%s</a></div>', OxoCore_Plugin::attributes( $read_more_wrapper_class ),  get_permalink(), $link_target,  apply_filters( 'aione_read_more_name', __( 'Read More', 'oxo-core' ) ) );

				if ( self::$args['layout'] == 'large-alternate' || 
					 self::$args['layout'] == 'medium-alternate'
				) {
					$inner_content = sprintf( '<div class="oxo-meta-info">%s</div>', $inner_content );
				}
	...
	function loop_content() {
		...
		$content = oxo_get_post_content( '', self::$args['excerpt'], self::$args['excerpt_words'], self::$args['strip_html'] );
		
		$content = sprintf( '<div class="oxo-post-content-container">%s</div>', $content );
...
add_action('pre_get_posts', 'oxo_query_offset', 1 );
function oxo_query_offset( &$query ) {
...
add_filter('found_posts', 'oxo_adjust_offset_pagination', 1, 2 );
function oxo_adjust_offset_pagination( $found_posts, $query ) {
...
add_filter( 'redirect_canonical', 'oxo_blog_redirect_canonical' );
function oxo_blog_redirect_canonical( $redirect_url ) {
