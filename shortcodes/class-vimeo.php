...
class OxoSC_Vimeo {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_vimeo-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_vimeo-shortcode-video-sc', array( $this, 'video_sc_attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		$html = sprintf( '<div %s><div %s><iframe src="%s://player.vimeo.com/video/%s?autoplay=0%s" width="%s" height="%s" frameborder="0" allowfullscreen></iframe></div></div>',
						 OxoCore_Plugin::attributes( 'vimeo-shortcode' ), OxoCore_Plugin::attributes( 'vimeo-shortcode-video-sc' ), $protocol, $id, $api_params, $width, $height );
	...
	function attr() {
		...
		$attr['class'] = 'oxo-video oxo-vimeo';
