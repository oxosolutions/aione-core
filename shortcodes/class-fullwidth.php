<?php
	...
	class OxoSC_FullWidth {
		...
		public function __construct() {
			...
			add_filter( 'oxo_attr_fullwidth-shortcode', array( $this, 'attr' ) );
			add_filter( 'oxo_attr_fullwidth-shortcode-oxo-row', array( $this, 'oxo_row' ) );
			add_filter( 'oxo_attr_fullwidth-overlay', array( $this, 'overlay_attr' ) );
			add_filter( 'oxo_attr_fullwidth-parallax', array( $this, 'parallax_attr' ) );
			add_filter( 'oxo_attr_fullwidth-faded', array( $this, 'faded_attr' ) );
		...
		public function ie9Detector() {
			echo "<!--[if IE 9]> <script>var _oxoParallaxIE9 = true;</script> <![endif]-->";
		}
		...
		function render( $args, $content = '' ) {
			...
			$c_pageID  = OxoCore_Plugin::c_pageID();
			...
			if ( is_page_template( '100-width.php' ) || is_page_template( 'blank.php' ) || ( '1' == oxo_get_option( 'portfolio_width_100', 'portfolio_width_100', $c_pageID ) || 'yes' == oxo_get_option( 'portfolio_width_100', 'portfolio_width_100', $c_pageID ) && 'aione_portfolio' == get_post_type( $c_pageID ) ) || ( aione_is_portfolio_template() && 'yes' == get_post_meta( $c_pageID, 'pyre_portfolio_width_100', true ) ) || '100-width.php' == $page_template ) {
			...
			if ( wp_is_mobile() && self::$bg_type == 'video' && ! empty( self::$args['video_url'] ) ) {
				...
				if ( $overlay_color ) {
					$outer_html .= sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'fullwidth-overlay' ) );
				}
			...
			if ( self::$args['fade'] == 'yes' && self::$bg_type == "image" ) {
				self::$bg_type = 'faded';
				$outer_html .= sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'fullwidth-faded' ) );
			}
			...
			if ( self::$args['background_parallax'] != 'none' &&
				...
				$parallax_helper = sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'fullwidth-parallax' ) );
			}
            ...
            if( $defaults['hundred_percent'] == 'yes' && $width_100 == true ) {
                ...
                $styles .= '.oxo-fullwidth-' . $this->fwc_counter . ', .oxo-fullwidth-' . $this->fwc_counter . '.oxo-section-separator {
                ...
                if( ! $site_width_percent ) {
                    ...
                    $styles .= '.oxo-fullwidth-' . $this->fwc_counter . ' {
                            padding-left: ' . self::$args['padding_left'] . ' !important;
                            padding-right: ' . self::$args['padding_right'] . ' !important;
                        }';
			...
			$html = sprintf( '%s<div %s>%s<div %s>%s</div></div>', $parallax_helper, OxoCore_Plugin::attributes( 'fullwidth-shortcode' ), $styles . $outer_html, OxoCore_Plugin::attributes( 'fullwidth-shortcode-oxo-row' ), do_shortcode( $content ) );
		...
		function attr() {
			...
			$attr['class'] = 'oxo-fullwidth fullwidth-box oxo-fullwidth-' . $this->fwc_counter . ' ';
			$attr['style'] = '';
			
			$attr['class'] .= sprintf( ' oxo-parallax-%s', self::$args['background_parallax'] );
			...
			if ( self::$args['hundred_percent'] == 'yes' ) {
				...
				if ( (int) self::$args['padding_left'] > 0 || (int) self::$args['padding_right'] > 0 ) {
					$attr['class'] .= ' oxo-nonzero-padding';
				}
			...
			if ( self::$args['equal_height_columns'] == 'yes' ) {
				$attr['class'] .= ' oxo-equal-height-columns';
			}
			...
			if ( self::$args['padding_bottom'] ) {
				$attr['style'] .= sprintf( 'padding-bottom:%s;', Aione_Sanitize::get_value_with_unit( self::$args['padding_bottom'] ) );
			}

			if ( self::$args['padding_top'] ) {
				$attr['style'] .= sprintf( 'padding-top:%s;', Aione_Sanitize::get_value_with_unit( self::$args['padding_top'] ) );
			}
			
            if ( self::$args['padding_left'] ) {
                $attr['style'] .= sprintf( 'padding-left:%s;', Aione_Sanitize::get_value_with_unit( self::$args['padding_left'] ) );
            }

            if ( self::$args['padding_right'] ) {
                $attr['style'] .= sprintf( 'padding-right:%s;', Aione_Sanitize::get_value_with_unit( self::$args['padding_right'] ) );
            }

			if ( self::$args['hundred_percent'] == 'no' && (int) self::$args['padding_left'] > 0 && $width_100 == true ) {
				$attr['style'] .= sprintf( 'padding-left:%s !important;', Aione_Sanitize::get_value_with_unit( self::$args['padding_left'] ) );
			}

			if ( self::$args['hundred_percent'] == 'no' && (int) self::$args['padding_right'] > 0 && $width_100 == true ) {
				$attr['style'] .= sprintf( 'padding-right:%s !important;', Aione_Sanitize::get_value_with_unit( self::$args['padding_right'] ) );
			}
