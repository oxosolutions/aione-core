...
class OxoSC_PricingTable {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_pricingtable-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_pricingtable-shortcode-column-wrapper', array( $this, 'column_wrapper_attr' ) );	
		add_filter( 'oxo_attr_pricingtable-shortcode-price', array( $this, 'price_attr' ) );
		add_filter( 'oxo_attr_pricingtable-shortcode-row', array( $this, 'row_attr' ) );
		add_filter( 'oxo_attr_pricingtable-shortcode-footer', array( $this, 'footer_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		.pricing-table-{$this->pricing_table_counter} .oxo-panel, .pricing-table-{$this->pricing_table_counter} .panel-wrapper:last-child .oxo-panel, 
		.pricing-table-{$this->pricing_table_counter} .standout .oxo-panel, .pricing-table-{$this->pricing_table_counter}  .panel-heading, 
		...
		$html = sprintf( '%s<div %s>%s</div>', $styles, OxoCore_Plugin::attributes( 'pricingtable-shortcode' ), do_shortcode($content) );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-pricing-table pricing-table-%s %s-boxed-pricing row oxo-columns-%s columns-%s oxo-clearfix', 
	...
	function render_child_column( $args, $content = '') {
		...
		$html = sprintf( '<div %s><div %s><div %s><div %s><h3 %s>%s</h3></div>%s', OxoCore_Plugin::attributes( 'pricingtable-shortcode-column-wrapper' ), 
						 OxoCore_Plugin::attributes( 'panel-container' ), OxoCore_Plugin::attributes( 'oxo-panel' ), OxoCore_Plugin::attributes( 'panel-heading' ), 
						 OxoCore_Plugin::attributes( 'title-row' ), $title, do_shortcode( $content ) );
	...
	function column_wrapper_attr() {
		...
		$attr['class'] = 'panel-wrapper oxo-column column col-lg-' . $columns . ' col-md-' . $columns . ' col-sm-' . $columns;
		
		if( self::$parent_args['columns'] == '5'  ) {
			$attr['class'] = 'panel-wrapper oxo-column column col-lg-2 col-md-2 col-sm-2';
		}
	...
	function render_child_price( $args, $content = '') {
		...
		if( isset( $price ) 
			...
			if( $currency_position != 'right' ){
				$pricing = sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'currency' ), $currency );
			}
			
			$pricing .= sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'integer-part' ), $price[0] );			
			
			if( array_key_exists( '1', $price ) ) {
				$pricing .= sprintf( '<sup %s>%s</sup>', OxoCore_Plugin::attributes( 'decimal-part' ), $price[1] );
			}
			...
			if( $currency_position == 'right' ){
				...
				$pricing .= sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( $currency_classes ), $currency );
				
				if( $time ) {
					$pricing .= sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( $time_classes ), $time );
				}				
			}			
			...
			$html = sprintf( '<div %s><div %s>%s</div></div>%s', OxoCore_Plugin::attributes( 'panel-body pricing-row' ), OxoCore_Plugin::attributes( 'price ' . $pricing_class ), 
							  $pricing, do_shortcode( $content ) );
			
		} else {
		
			$html = sprintf( '<div %s></div>%s', OxoCore_Plugin::attributes( 'panel-body pricing-row' ), do_shortcode( $content ) );
	...
	function render_child_row( $args, $content = '') {
		...
		if( $this->is_first_row ) {
			$html = sprintf( '<ul %s>', OxoCore_Plugin::attributes( 'list-group' ) );
			$this->is_first_row = false;
		}	
		
		$html .= sprintf( '<li %s>%s</li>', OxoCore_Plugin::attributes( 'list-group-item normal-row' ), do_shortcode( $content ) );
	...
	function render_child_footer( $args, $content = '') {
		...
		$html = sprintf( '</ul><div %s>%s</div>', OxoCore_Plugin::attributes( 'panel-footer footer-row' ), do_shortcode( $content ) );
