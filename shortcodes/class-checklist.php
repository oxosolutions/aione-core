...
class OxoSC_Checklist {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_checklist-shortcode', array( $this, 'attr' ) );
		add_shortcode( 'checklist', array( $this, 'render_parent' ) );

		add_filter( 'oxo_attr_checklist-shortcode-li-item', array( $this, 'li_attr' ) );
		add_filter( 'oxo_attr_checklist-shortcode-span', array( $this, 'span_attr' ) );
		add_filter( 'oxo_attr_checklist-shortcode-icon', array( $this, 'icon_attr' ) );
		add_filter( 'oxo_attr_checklist-shortcode-item-content', array( $this, 'item_content_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<ul %s>%s</ul>', OxoCore_Plugin::attributes( 'checklist-shortcode' ), do_shortcode( $content ) );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-checklist oxo-checklist-%s', $this->checklist_counter );
	...
	function render_child( $args, $content = '') {
		...
		$html =  sprintf( '<li %s><span %s><i %s></i></span><div %s>%s</div></li>', OxoCore_Plugin::attributes( 'checklist-shortcode-li-item' ), OxoCore_Plugin::attributes( 'checklist-shortcode-span' ),
						  OxoCore_Plugin::attributes( 'checklist-shortcode-icon' ), OxoCore_Plugin::attributes( 'checklist-shortcode-item-content' ), do_shortcode( $content ) );
	...
	function item_content_attr() {
		...
		$attr['class'] = 'oxo-li-item-content';
	...
	function icon_attr() {
		...
		if( ! self::$child_args['icon'] ) {
			$icon = OxoCore_Plugin::font_awesome_name_handler( self::$parent_args['icon'] );
		} else {
			$icon = OxoCore_Plugin::font_awesome_name_handler( self::$child_args['icon'] );
		}
		...
		$attr['class'] = sprintf( 'oxo-li-icon fa %s', $icon );
