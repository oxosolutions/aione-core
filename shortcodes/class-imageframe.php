...
class OxoSC_Imageframe {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_imageframe-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_imageframe-shortcode-link', array( $this, 'link_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$rgb = OxoCore_Plugin::hex2rgb( $stylecolor );
		...
		if ( array_key_exists( '2', $src ) ) {
			...
			$image_id = OxoCore_Plugin::get_attachment_id_from_url( $image_url );
		...
		if( $lightbox == 'yes' ) {
			...
			$output = sprintf( '<a %s>%s</a>', OxoCore_Plugin::attributes( 'imageframe-shortcode-link' ), do_shortcode( $content ) );
		} elseif( $link ) {
			$output = sprintf( '<a %s>%s</a>', OxoCore_Plugin::attributes( 'imageframe-shortcode-link' ), do_shortcode( $content ) );
		}

		$html = sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'imageframe-shortcode' ), $output );
		...
		if( $hover_type == 'liftup' ) {
			...
			$html = sprintf( '<div %s>%s%s</div>', OxoCore_Plugin::attributes( $liftup_classes ), $liftup_styles, $html );
		}

		if( $align == 'center' ) {
			$html = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'imageframe-align-center' ), $html );
		}
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-imageframe imageframe-%s imageframe-%s', self::$args['style'], $this->imageframe_counter );
