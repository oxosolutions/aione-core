...
class OxoSC_Tagline {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_tagline-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_tagline-shortcode-reading-box', array( $this, 'reading_box_attr' ) );
		add_filter( 'oxo_attr_tagline-shortcode-button', array( $this, 'button_attr' ) );
	...
	function render( $args, $content = '') {
		...
		if( ( isset( $link ) && $link ) && 
			...
			self::$args['button_class'] = ' oxo-desktop-button continue';
			$additional_content = sprintf( '<a %s><span>%s</span></a>', OxoCore_Plugin::attributes( 'tagline-shortcode-button' ), $button );
		}
		...
		if( ( isset( $link ) && $link ) && ( isset( $button ) && $button ) ) {
			self::$args['button_class'] = ' oxo-mobile-button';
			$additional_content .= sprintf( '<a %s><span>%s</span></a>', OxoCore_Plugin::attributes( 'tagline-shortcode-button' ), $button );
		}

		$html = sprintf('%s<div %s><div %s>%s</div></div>', $styles, OxoCore_Plugin::attributes( 'tagline-shortcode' ), OxoCore_Plugin::attributes( 'tagline-shortcode-reading-box' ), $additional_content );
	...
	function attr() {
		...
		$attr['class'] = 'oxo-reading-box-container reading-box-container-' . $this->tagline_box_counter;
	...
	function button_attr() {
		...
		$attr['class'] = sprintf( 'button oxo-button button-%s button-%s oxo-button-%s button-%s button-%s%s', self::$args['buttoncolor'],
