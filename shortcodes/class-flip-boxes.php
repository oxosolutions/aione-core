...
class OxoSC_FlipBoxes {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_flip-boxes-shortcode', array( $this, 'parent_attr' ) );
		add_shortcode( 'flip_boxes', array( $this, 'render_parent' ) );

		add_filter( 'oxo_attr_flip-box-shortcode', array( $this, 'child_attr' ) );
		add_filter( 'oxo_attr_flip-box-shortcode-front-box', array( $this, 'front_box_attr' ) );
		add_filter( 'oxo_attr_flip-box-shortcode-back-box', array( $this, 'back_box_attr' ) );
		add_filter( 'oxo_attr_flip-box-shortcode-heading-front', array( $this, 'heading_front_attr' ) );
		add_filter( 'oxo_attr_flip-box-shortcode-heading-back', array( $this, 'heading_back_attr' ) );
		add_filter( 'oxo_attr_flip-box-shortcode-grafix', array( $this, 'grafix_attr' ) );		
		add_filter( 'oxo_attr_flip-box-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<div %s>%s</div><div %s></div>', OxoCore_Plugin::attributes( 'flip-boxes-shortcode' ), do_shortcode( $content ), OxoCore_Plugin::attributes( 'oxo-clearfix' ) );
	...
	function parent_attr() {
		...
		$attr['class'] = sprintf( 'oxo-flip-boxes flip-boxes row oxo-columns-%s', self::$parent_args['columns'] );
	...
	function render_child( $args, $content = '') {
		...
		if( $image && 
			...
			$icon_output = sprintf( '<i %s></i>', OxoCore_Plugin::attributes( 'flip-box-shortcode-icon' ) );
		}
		
		if( $icon_output ) {
			$icon_output = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'flip-box-shortcode-grafix' ), $icon_output );
		} else {
			$icon_output = '';
		}

		if( $title_front ) {
			$title_front_output = sprintf( '<h2 %s>%s</h2>', OxoCore_Plugin::attributes( 'flip-box-shortcode-heading-front' ), $title_front );
		}
		
		if( $title_back ) {
			$title_back_output = sprintf( '<h3 %s>%s</h3>', OxoCore_Plugin::attributes( 'flip-box-shortcode-heading-back' ), $title_back );
		}		

		$front_inner = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'flip-box-front-inner' ), $icon_output . $title_front_output . $text_front );
		$back_inner = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'flip-box-back-inner' ), $title_back_output . do_shortcode( $content ) );

		$front = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'flip-box-shortcode-front-box' ), $front_inner );
		$back = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'flip-box-shortcode-back-box' ), $back_inner );

		$html = sprintf( '<div %s><div class="oxo-flip-box"><div %s>%s%s</div></div></div>', OxoCore_Plugin::attributes( 'flip-box-shortcode' ), OxoCore_Plugin::attributes( 'flip-box-inner-wrapper' ), $front, $back );
	...
	function child_attr() {
		...
		$attr['class'] = sprintf('oxo-flip-box-wrapper oxo-column col-lg-%s col-md-%s col-sm-%s', $columns, $columns, $columns );
		
		if( self::$parent_args['columns'] == '5'  ) {
			$attr['class'] = 'oxo-flip-box-wrapper col-lg-2 col-md-2 col-sm-2';
		}			
	...
	function icon_attr() {
		...
		if( self::$child_args['image'] ) {
			...
			$attr['class'] = sprintf( 'fa %s', OxoCore_Plugin::font_awesome_name_handler( self::$child_args['icon'] ) );
		}
