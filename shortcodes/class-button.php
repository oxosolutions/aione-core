...
class OxoSC_Button {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_button-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_button-shortcode-icon-divder', array( $this, 'icon_divider_attr' ) );
		add_filter( 'oxo_attr_button-shortcode-icon', array( $this, 'icon_attr' ) );
		add_filter( 'oxo_attr_button-shortcode-button-text', array( $this, 'button_text_attr' ) );
	...
	function render( $args, $content = '') {
		...
		if( ( $color == 'custom' || $color == 'default' ) &&
            ...
            if ( $text_color_styles ) {
                $styles .= sprintf( '.oxo-button.button-%s .oxo-button-text, .oxo-button.button-%s i {%s}', $this->button_counter, $this->button_counter, $text_color_styles );
			}

			if ( $general_styles ) {
				$styles .= sprintf( '.oxo-button.button-%s {%s}', $this->button_counter, $general_styles );
			}

			if ( $accent_color ) {
				$styles .= sprintf( '.oxo-button.button-%s .oxo-button-icon-divider{border-color:%s;}', $this->button_counter, $accent_color );
			}

			if( $button_3d_styles ) {
				$styles .= sprintf( '.oxo-button.button-%s.button-3d{%s}.button-%1$s.button-3d:active{%2$s}', $this->button_counter, $button_3d_styles );
			}
			
            if ( $text_color_hover_styles ) {
                $styles .= sprintf( '.oxo-button.button-%s:hover .oxo-button-text, .oxo-button.button-%s:hover i,.oxo-button.button-%s:focus .oxo-button-text, .oxo-button.button-%s:focus i,.oxo-button.button-%s:active .oxo-button-text, .oxo-button.button-%s:active{%s}', 
			...
			if( $hover_styles ) {
				$styles .= sprintf( '.oxo-button.button-%s:hover, .oxo-button.button-%s:focus, .oxo-button.button-%s:active{%s}', $this->button_counter, $this->button_counter, $this->button_counter, $hover_styles );
			}
			
			if ( $accent_hover_color ) {
				$styles .= sprintf( '.oxo-button.button-%s:hover .oxo-button-icon-divider, .oxo-button.button-%s:hover .oxo-button-icon-divider, .oxo-button.button-%s:active .oxo-button-icon-divider{border-color:%s;}', $this->button_counter, $this->button_counter, $this->button_counter, $accent_hover_color );
			}			
			...
			if( $gradient_colors ) {
				...
				$styles .= sprintf( '.oxo-button.button-%s{%s}', $this->button_counter, $gradient_styles );
			}
			...
			if( $gradient_hover_colors ) {
				...
				$styles .= sprintf( '.oxo-button.button-%s:hover,.button-%s:focus,.oxo-button.button-%s:active{%s}', $this->button_counter, $this->button_counter, $this->button_counter, $gradient_hover_styles );
			}
		}

		if( self::$args['stretch'] == 'yes' ) {
	   		$styles .= sprintf( '.oxo-button.button-%s{width:100%%;}', $this->button_counter );
	   	} else if ( self::$args['stretch'] == 'no' ) {
	   		$styles .= sprintf( '.oxo-button.button-%s{width:auto;}', $this->button_counter );
	   	}
		...
		$icon_html = '';
		if( $icon ) {
			$icon_html = sprintf( '<i %s></i>', OxoCore_Plugin::attributes( 'button-shortcode-icon' ) );

			if( $icon_divider == 'yes' ) {
				$icon_html = sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'button-shortcode-icon-divder' ), $icon_html );
			}
		}

		$button_text = sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'button-shortcode-button-text' ), do_shortcode( $content ) );
		...
		$html = sprintf( '%s<a %s>%s</a>', $style_tag, OxoCore_Plugin::attributes( 'button-shortcode' ), $inner_content );

		// Add wrapper to the button for alignment and scoped styling
		if ( $alignment ) {
			$alignment = ' oxo-align' . $alignment;
		}

		$html = sprintf( '<div class="oxo-button-wrapper%s">%s</div>', $alignment, $html );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-button button-%s button-%s button-%s button-%s button-%s', self::$args['type'], self::$args['shape'], self::$args['size'], self::$args['color'], $this->button_counter );
	...
	function icon_divider_attr() {
		...
		$attr['class'] = sprintf( 'oxo-button-icon-divider button-icon-divider-%s', self::$args['icon_position'] );
	...
	function icon_attr() {
		...
		$attr['class'] = sprintf( 'fa %s', OxoCore_Plugin::font_awesome_name_handler( self::$args['icon'] ) );
	...
	function button_text_attr() {
		...
		if( self::$args['icon'] &&
			...
			$attr['class'] = sprintf( 'oxo-button-text oxo-button-text-%s', self::$args['icon_position'] );

 		} else {
 			$attr['class'] = 'oxo-button-text';
 		}
