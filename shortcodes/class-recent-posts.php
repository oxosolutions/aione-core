...
class OxoSC_RecentPosts {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_recentposts-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_recentposts-shortcode-section', array( $this, 'section_attr' ) );
		add_filter( 'oxo_attr_recentposts-shortcode-column', array( $this, 'column_attr' ) );		
		add_filter( 'oxo_attr_recentposts-shortcode-slideshow', array( $this, 'slideshow_attr' ) );
		add_filter( 'oxo_attr_recentposts-shortcode-img', array( $this, 'img_attr' ) );
		add_filter( 'oxo_attr_recentposts-shortcode-img-link', array( $this, 'link_attr' ) );
	...
	function render( $args, $content = '') {
		...
		while( $recent_posts->have_posts() ) {
			...
			if( $layout == 'date-on-side' ) {
				...
				$date_box = sprintf( '<div %s><div %s><span %s>%s</span><span %s>%s</span></div><div %s><i %s></i></div></div>', OxoCore_Plugin::attributes( 'oxo-date-and-formats' ), 
									 OxoCore_Plugin::attributes( 'oxo-date-box updated' ), OxoCore_Plugin::attributes( 'oxo-date' ), get_the_time( $smof_data['alternate_date_format_day'] ), 
									 OxoCore_Plugin::attributes( 'oxo-month-year' ), get_the_time( $smof_data['alternate_date_format_month_year'] ), OxoCore_Plugin::attributes( 'oxo-format-box' ), 
							 		 OxoCore_Plugin::attributes( 'oxo-icon-' . $format_class ) );
			}
			...
			if( $thumbnail == 'yes' &&
				...
				if( has_post_thumbnail() || 
					...
					if( get_post_meta( get_the_ID(), 'pyre_video', true ) ) {
						$slides .= sprintf( '<li><div %s>%s</div></li>', OxoCore_Plugin::attributes( 'full-video' ), get_post_meta(get_the_ID(), 'pyre_video', true) );
					}
					...
					if( has_post_thumbnail() ) {
						...
						$slides .= sprintf( '<li><a href="%s" %s>%s</a></li>', get_permalink( get_the_ID() ),
											OxoCore_Plugin::attributes( 'recentposts-shortcode-img-link' ), 
					...
					$slideshow = sprintf( '<div %s><ul %s>%s</ul></div>', OxoCore_Plugin::attributes( 'recentposts-shortcode-slideshow' ), OxoCore_Plugin::attributes( 'slides' ), $slides );
				}
			}

			if( $title == 'yes' ) {
				$content .= aione_render_rich_snippets_for_pages( false );
				$entry_title = '';
				...
				$content .= aione_render_rich_snippets_for_pages();
			}
			...
			if( $meta == 'yes' ) {
				...
				ob_start();
				comments_popup_link( __( '0 Comments', 'oxo-core' ), __( '1 Comment', 'oxo-core' ), '% ' . __( 'Comments', 'oxo-core' ) );
				$comments_link = ob_get_contents();
				ob_get_clean();				
				
				$comments = sprintf( '<span %s>|</span><span>%s</span>', OxoCore_Plugin::attributes( 'meta-separator' ), $comments_link );	

				$content .= sprintf( '<p %s><span><span %s>%s</span></span>%s</p>', OxoCore_Plugin::attributes( 'meta' ), OxoCore_Plugin::attributes( 'date' ), 
									 get_the_time( $smof_data['date_format'], get_the_ID() ), $comments);

			}

			if( $excerpt == 'yes' ) {		
				$content .= oxo_get_post_content( '', 'yes', $excerpt_words, $strip_html );
			}
						
			if( $count == self::$args['columns'] ) {
				$count = 0;
				$items .= sprintf( '<div %s>%s%s<div %s>%s</div></div><div class="oxo-clearfix"></div>', OxoCore_Plugin::attributes( 'recentposts-shortcode-column' ), $date_box, $slideshow, 
							   OxoCore_Plugin::attributes( 'recent-posts-content' ), $content );
			} else {
				$items .= sprintf( '<div %s>%s%s<div %s>%s</div></div>', OxoCore_Plugin::attributes( 'recentposts-shortcode-column' ), $date_box, $slideshow, 
							   OxoCore_Plugin::attributes( 'recent-posts-content' ), $content );
			}
		...
		$html = sprintf( '<div %s><section %s>%s</section></div>', OxoCore_Plugin::attributes( 'recentposts-shortcode' ), 
					  OxoCore_Plugin::attributes( 'recentposts-shortcode-section' ), $items );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-recent-posts aione-container layout-%s layout-columns-%s', self::$args['layout'], self::$args['columns'] );
	...
	function section_attr() {
		...
		$attr['class'] = sprintf( 'oxo-columns columns oxo-columns-%s columns-%s', self::$args['columns'], self::$args['columns'] );
	...
	function column_attr() {
		...
		$attr['class'] = sprintf( 'oxo-column column col col-lg-%s col-md-%s col-sm-%s', $columns, $columns, $columns );
		
		if( self::$args['columns'] == '5'  ) {
			$attr['class'] = 'oxo-column column col-lg-2 col-md-2 col-sm-2';
		}
	...
	function slideshow_attr() {
		...
		$attr['class'] = 'oxo-flexslider flexslider';
