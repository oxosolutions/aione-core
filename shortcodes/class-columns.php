...
class OxoSC_Columns {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_columns-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_columns-shortcode-wrapper', array( $this, 'wrapper_attr' ) );
		add_filter( 'oxo_attr_columns-shortcode-inner-bg', array( $this, 'inner_bg_attr' ) );
	...
	function render( $args, $content = '', $tag ) {
		...
		$clearfix = '';
		if ( $last == 'yes' ) {
			$clearfix = sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'oxo-clearfix' ) );
		}	

		$inner_content = do_shortcode( $content );
		
		$inner_content .= '<div class="oxo-clearfix"></div>';

		// If content should be centered, add needed markup
		if ( $center_content == 'yes' ) {
			$inner_content = sprintf( '<div class="oxo-column-table"><div class="oxo-column-tablecell">%s</div></div>', $inner_content );
		}
		...
		if( $link || ( $hover_type && $hover_type != 'none' ) ) {
			...
			$inner_bg = sprintf( '<span class="oxo-column-inner-bg hover-type-%s"><a %s><span %s></span></a></span>', $hover_type, $bg_link, OxoCore_Plugin::attributes( 'columns-shortcode-inner-bg', $defaults ) );
		}

		// Setup the main markup
		$html = sprintf( '<div %s><div %s>%s</div>%s</div>%s', OxoCore_Plugin::attributes( 'columns-shortcode', $defaults ), OxoCore_Plugin::attributes( 'columns-shortcode-wrapper', $defaults ), $inner_content, $inner_bg, $clearfix );
	...
	function attr( $args ) {
		...
		$attr['class'] = 'oxo-' . $class . ' oxo-layout-column';
		
		if( $args['link'] || ( $args['hover_type'] && $args['hover_type'] != 'none' ) ) {
			$attr['class'] .= ' oxo-column-inner-bg-wrapper';
		}

		// Set the last class on the rightmost column to supress margin
		if ( $args['last'] == 'yes' ) {
			$attr['class'] .= ' oxo-column-last';
		}

		// Set spacing class for correct widths
		$attr['class'] .= ' oxo-spacing-' . $args['spacing'];

		$attr['style'] = sprintf( 'margin-top:%s;margin-bottom:%s;', Aione_Sanitize::get_value_with_unit( $args['margin_top'] ), Aione_Sanitize::get_value_with_unit( $args['margin_bottom'] ) );
	...
	function wrapper_attr( $args ) {
		...
		// Set custom padding
		if ( $args['padding'] ) {
			$attr['style'] .= sprintf( 'padding:%s;', Aione_Sanitize::get_value_with_unit( $args['padding'] ) );
		}
	...
	function inner_bg_attr( $args ) {
		...
		$attr['class'] = 'oxo-column-inner-bg-image';
		$attr['style'] = '';
