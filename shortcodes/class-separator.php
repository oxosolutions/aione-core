...
class OxoSC_Separator {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_separator-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_separator-shortcode-icon-wrapper', array( $this, 'icon_wrapper_attr' ) );
		add_filter( 'oxo_attr_separator-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render( $args, $content = '') {
		...
		if ( $icon && 
			...
			$icon_insert = sprintf( '<span %s><i %s></i></span>', OxoCore_Plugin::attributes( 'separator-shortcode-icon-wrapper' ), OxoCore_Plugin::attributes( 'separator-shortcode-icon' ) );
		} else {
			$icon_insert = '';
		}

		$html = sprintf( '<div %s></div><div %s>%s</div>', OxoCore_Plugin::attributes( 'oxo-sep-clear' ), OxoCore_Plugin::attributes( 'separator-shortcode' ), $icon_insert );

		if ( self::$args['alignment'] == 'right' ) {
			$html .= sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'oxo-sep-clear' ) );
		}
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-separator' );
		$attr['style'] = '';


		if( ! self::$args['width'] || self::$args['width'] == '100%' ) {
			$attr['class'] .= ' oxo-full-width-sep';
		}
		...
		if ( self::$args['alignment'] == 'center' ) {
			...
			$attr['class'] .=  ' oxo-clearfix';
		}
		
		$attr['style'] .= sprintf( 'margin-top:%s;', Aione_Sanitize::get_value_with_unit( self::$args['top_margin'] ) );		
	
		if( self::$args['bottom_margin'] ) {
			$attr['style'] .= sprintf( 'margin-bottom:%s;', Aione_Sanitize::get_value_with_unit( self::$args['bottom_margin'] ) );
		}
