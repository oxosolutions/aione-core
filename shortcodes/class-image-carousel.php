...
class OxoSC_ImageCarousel {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_image-carousel-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_image-carousel-shortcode-carousel', array( $this, 'carousel_attr' ) );
		add_filter( 'oxo_attr_image-carousel-shortcode-slide-link', array( $this, 'slide_link_attr' ) );
		add_filter( 'oxo_attr_oxo-image-wrapper', array( $this, 'image_wrapper' ) );
		//add_filter( 'oxo_attr_oxo-nav-prev', array( $this, 'oxo_nav_prev' ) );
		//add_filter( 'oxo_attr_oxo-nav-next', array( $this, 'oxo_nav_next' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<div %s>', OxoCore_Plugin::attributes( 'image-carousel-shortcode' ) );
			$html .= sprintf( '<div %s>', OxoCore_Plugin::attributes( 'image-carousel-shortcode-carousel' ) );
	...
	function attr() {
		...
		$attr['class'] = 'oxo-image-carousel oxo-image-carousel-' . self::$parent_args['picture_size'];		
		...
		if( self::$parent_args['border'] == 'yes' ) {
		  $attr['class'] .= ' oxo-carousel-border';
		}		
	...
	function render_child( $args, $content = '') {
		...
		if( self::$parent_args['mouse_scroll'] == 'no' &&
			...
			$output = sprintf( '<a %s>%s</a>', OxoCore_Plugin::attributes( 'image-carousel-shortcode-slide-link' ), $output );
		}

		$html = sprintf( '<li %s><div %s><div %s>%s</div></div></li>', OxoCore_Plugin::attributes( 'oxo-carousel-item'), OxoCore_Plugin::attributes( 'oxo-carousel-item-wrapper'), 
						 OxoCore_Plugin::attributes( 'oxo-image-wrapper' ), $output );
		return $html;
	...
	function oxo_nav_prev() {

		$attr = array();

		$attr['class'] = 'oxo-nav-prev oxo-icon-left';

		return $attr;

	}

	function oxo_nav_next() {

		$attr = array();

		$attr['class'] = 'oxo-nav-next  oxo-icon-right';
