...
class OxoSC_RecentWorks {
	...
	public function __construct() {
		...
		// Actions
		add_action( 'oxo_recent_works_shortcode_content', array( $this, 'get_post_content' ) );

		// Element attributes
		add_filter( 'oxo_attr_recentworks-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_recentworks-shortcode-portfolio-wrapper', array( $this, 'portfolio_wrapper_attr' ) );
		add_filter( 'oxo_attr_recentworks-shortcode-carousel', array( $this, 'carousel_attr' ) );
		add_filter( 'oxo_attr_recentworks-shortcode-slideshow', array( $this, 'slideshow_attr' ) );
		add_filter( 'oxo_attr_recentworks-shortcode-filter-link', array( $this, 'filter_link_attr' ) );
	...
	function render( $args, $content = '') {
		...
		while ( $recent_works->have_posts() ) { 
			...
			if ( $layout == 'carousel' ) {
				...
				// Title below image layout
					...
					// Get the post title
					$title_terms .= sprintf( '<h4 %s><a href="%s" target="%s">%s</a></h4>', OxoCore_Plugin::attributes( 'oxo-carousel-title' ), get_permalink( get_the_ID() ), '_self', get_the_title() );

					// Get the terms
					$title_terms .= get_the_term_list( get_the_ID(), 'portfolio_category', '<div class="oxo-carousel-meta">', ', ', '</div>' );					
				}
				
				// Render the video set in page options if no featured image is present
				if ( ! has_post_thumbnail() &&
					 oxo_get_page_option( 'video', get_the_ID() )
				) {			
					...
					// For all other layouts get the calculated max-width from the image size
						...
						$featured_image_size_dimensions = aione_get_image_size_dimensions( $this->image_size );
						$video_max_width = $featured_image_size_dimensions['width'];
					}				
				
					$image = sprintf( '<div class="oxo-image-wrapper oxo-video" style="max-width:%s;">%s</div>', $video_max_width, oxo_get_page_option( 'video', get_the_ID() ) );
				} else {
					// Get the post image
					$image = aione_render_first_featured_image_markup( get_the_ID(), $this->image_size, get_permalink( get_the_ID() ), TRUE, FALSE, FALSE, 'disable', $show_title, '', $gallery_id );
				}
				
				$portfolio_posts .= sprintf( '<li %s><div %s>%s%s%s</div></li>', OxoCore_Plugin::attributes( 'oxo-carousel-item'), OxoCore_Plugin::attributes( 'oxo-carousel-item-wrapper' ), 
											 aione_render_rich_snippets_for_pages(), $image, $title_terms );
				...
				if ( ! has_post_thumbnail() &&
					...
					// Get the post image
					$image = aione_render_first_featured_image_markup( get_the_ID(), $this->image_size, get_permalink( get_the_ID() ), TRUE, FALSE, FALSE, 'disable', 'default', '', $gallery_id );
				}
				...
				if( $layout == 'grid-with-excerpts' ) {
					...
					$rich_snippets = aione_render_rich_snippets_for_pages( false );
					
					// Get the post title
					$post_title = aione_render_post_title( get_the_ID() );
					...
					ob_start();
					/**
					 * oxo_recent_works_shortcode_content hook
					 *
					 * @hooked content - 10 (outputs the post content)
					 */						
					do_action( 'oxo_recent_works_shortcode_content' );					
					...
					if ( $boxed_text == 'boxed' &&
						...
						$separator = '<div class="oxo-content-sep"></div>';
					}

					// On one column layouts render the "Learn More" and "View Project" buttons
					if ( $columns == '1' ) {
						$classes = sprintf( 'oxo-button oxo-button-small oxo-button-default oxo-button-%s oxo-button-%s', 
											strtolower( $smof_data['button_shape'] ), strtolower( $smof_data['button_type'] ) );
						
						// Add the "Learn More" button
						$learn_more_button = sprintf( '<a href="%s" %s>%s</a>', $permalink, OxoCore_Plugin::attributes( $classes ), __( 'Learn More', 'oxo-core' ) );

						// If there is a project url, add the "View Project" button
						$view_project_button = '';
						if ( oxo_get_page_option( 'project_url', get_the_ID() ) ) {
							$view_project_button = sprintf( '<a href="%s" %s>%s</a>', oxo_get_page_option( 'project_url', get_the_ID() ), 
															OxoCore_Plugin::attributes( $classes ), __( 'View Project', 'oxo-core' ) );
						}

						// Wrap buttons
						$buttons = sprintf( '<div %s>%s%s</div>', OxoCore_Plugin::attributes( 'oxo-portfolio-buttons' ), $learn_more_button, $view_project_button );
					}
					
					

					// Put it all together
					$post_content = sprintf( '<div %s>%s%s%s<div %s>%s%s</div></div>', OxoCore_Plugin::attributes( 'oxo-portfolio-content' ), $post_title, $post_terms, $separator, 
											 OxoCore_Plugin::attributes( 'oxo-post-content' ), $stripped_content, $buttons );
				} else {
					// Get the rich snippets for grid layout without excerpts
					$rich_snippets = aione_render_rich_snippets_for_pages();
				}

				$portfolio_posts .= sprintf( '<div %s><div %s>%s%s%s</div></div>', OxoCore_Plugin::attributes( 'oxo-portfolio-post ' . $post_classes ), 
											 OxoCore_Plugin::attributes( 'oxo-portfolio-content-wrapper' ), $rich_snippets, $image, $post_content );
			}
		} // end while
		wp_reset_query();
		...
		// Carousel layout
		if( $layout == 'carousel' ) {
			$main_carousel = sprintf( '<ul %s>%s</ul>', OxoCore_Plugin::attributes( 'oxo-carousel-holder' ), $portfolio_posts );
						
			// Check if navigation should be shown
			$navigation = '';
			if ( $show_nav == 'yes' ) {
				$navigation = sprintf( '<div %s><span %s></span><span %s></span></div>', OxoCore_Plugin::attributes( 'oxo-carousel-nav' ),
									   OxoCore_Plugin::attributes( 'oxo-nav-prev' ), OxoCore_Plugin::attributes( 'oxo-nav-next' ) );
			}
								   
			$html = sprintf( '<div %s><div %s><div %s>%s%s</div></div></div>', OxoCore_Plugin::attributes( 'recentworks-shortcode' ), OxoCore_Plugin::attributes( 'recentworks-shortcode-carousel' ), OxoCore_Plugin::attributes( 'oxo-carousel-positioner' ), $main_carousel, $navigation );								   
		...
		// Grid layouts
			...
			if( $portfolio_categories && 
				...
				if ( $filters != 'yes-without-all' ) {
					$filter = sprintf( '<li %s><a %s>%s</a></li>', OxoCore_Plugin::attributes( 'oxo-filter oxo-filter-all oxo-active' ),
									   OxoCore_Plugin::attributes( 'recentworks-shortcode-filter-link', array( 'data-filter' => '*' ) ), __( 'All', 'oxo-core' ) );
					$first_filter = FALSE;
				} else {
					$first_filter = TRUE;
				}					
					...
					if ( ! in_array( $portfolio_category->slug, $cats_to_exclude ) ) {						
						...
						if ( ! empty( self::$args['cat_slug'] ) ) {
							...
							if ( in_array( $portfolio_category->slug, $cat_slugs ) ) {			
								...
								$filter .= sprintf( '<li %s><a %s>%s</a></li>', OxoCore_Plugin::attributes( 'oxo-filter oxo-hidden' . $active_class ), 
													OxoCore_Plugin::attributes( 'recentworks-shortcode-filter-link', array( 'data-filter' => '.' . $portfolio_category->slug ) ), $portfolio_category->name );
							}							
				...
				// Wrap filters
				$filter_wrapper = sprintf( '<ul %s>%s</ul>', OxoCore_Plugin::attributes( 'oxo-filters' ), $filter );

			}
			
			// For column spacing set needed css
			if ( $column_spacing ) {
				$styles = sprintf( '<style type="text/css">.oxo-recent-works-%s .oxo-portfolio-wrapper .oxo-col-spacing{padding:%spx;}</style>', $this->recent_works_counter, $column_spacing / 2 );
			}
		
			// Put it all together
			$html = sprintf( '<div %s>%s%s<div %s>%s</div></div>', OxoCore_Plugin::attributes( 'recentworks-shortcode' ), $filter_wrapper, $styles,
							 OxoCore_Plugin::attributes( 'recentworks-shortcode-portfolio-wrapper' ), $portfolio_posts );
	...
	function attr() {
		...
		// Add general classes
		$attr['class'] = sprintf( 'oxo-recent-works oxo-recent-works-%s oxo-recent-works-%s', $this->recent_works_counter, self::$args['layout'] );
		...
		// Add classes for grid layouts
			...
			$attr['class'] .= sprintf( ' oxo-portfolio oxo-portfolio-%s oxo-portfolio-%s', $this->column, self::$args['boxed_text'] );
			
			
			if ( self::$args['layout'] == 'grid-with-excerpts' ) {
				$attr['class'] .= ' oxo-portfolio-text';
			}			
	...
	function portfolio_wrapper_attr( $args ) {
		...
		$attr['class'] = 'oxo-portfolio-wrapper';
		
		$attr['id'] = sprintf( 'oxo-recent-works-%s', $this->recent_works_counter );
	...
	function carousel_attr() {	
		...
		if ( self::$args['picture_size'] == 'fixed' ) {
			$attr['class'] .= ' oxo-recent-works-carousel-fixed';
		}
	...
	function get_post_content() {
		echo oxo_get_post_content( '', 'yes', self::$args['excerpt_length'], self::$args['strip_html'] )	;
	}
}

new OxoSC_RecentWorks();