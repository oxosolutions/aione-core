...
class OxoSC_Popover {
	...
	add_filter( 'oxo_attr_popover-shortcode', array( $this, 'attr' ) );
	...
	function render( $args, $sc_content = '' ) {
		...
		$html = sprintf( '<span %s>%s%s</span>', OxoCore_Plugin::attributes( 'popover-shortcode' ), $styles, do_shortcode( $sc_content ) );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-popover popover-%s', $this->popover_counter );
