...
class OxoSC_SharingBox {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_sharingbox-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_sharingbox-shortcode-tagline', array( $this, 'tagline_attr' ) );
		add_filter( 'oxo_attr_sharingbox-shortcode-social-networks', array( $this, 'social_networks_attr' ) );
		add_filter( 'oxo_attr_sharingbox-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render( $args, $content = '') {
		...
		for( $i = 0; $i < count( $social_networks ); $i++ ) {
			...
			$icons .= sprintf( '<a %s></a>', OxoCore_Plugin::attributes( 'sharingbox-shortcode-icon', $icon_options ) );
		}

		$html = sprintf( '<div %s><h4 %s>%s</h4><div %s>%s</div></div>', OxoCore_Plugin::attributes( 'sharingbox-shortcode' ), OxoCore_Plugin::attributes( 'sharingbox-shortcode-tagline' ), 
						 $tagline, OxoCore_Plugin::attributes( 'sharingbox-shortcode-social-networks' ), $icons );
	...
	function attr() {
		...
		$attr['class'] = 'share-box oxo-sharing-box';
