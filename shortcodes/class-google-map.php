...
class OxoSC_GoogleMap {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_google-map-shortcode', array( $this, 'attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		if( $address ) {
			...
			if( $map_style == 'theme' ) {
				...
				$animation = 'yes';
				$infobox = 'custom';
				$infobox_background_color = OxoCore_Plugin::hex2rgb( $smof_data['primary_color'] );
				...
				$brightness_level = OxoCore_Plugin::calc_color_brightness( $smof_data['primary_color'] );
			...
			if( $icon == 'theme' && $map_style == 'custom' ) {
				for( $i = 0; $i < $num_of_addresses; $i++ ) {
					$icon_array[$i] = plugins_url( 'images/aione_map_marker.png', dirname( __FILE__ ) );				
				}
			}			
			...
			$cached_addresses = get_option( 'oxo_map_addresses' );
			...
			$map_id = uniqid( 'oxo_map_' ); // generate a unique ID for this map
			$this->map_id = $map_id;
			...
			<script type="text/javascript">
				...
				var markers = [];
				var counter = 0;
				function oxo_run_map_<?php echo $map_id ; ?>() {
					jQuery('#<?php echo $map_id ; ?>').oxo_maps({
						...
						overlay_color_hsl: <?php echo json_encode( OxoCore_Plugin::rgb2hsl( $overlay_color ) ); ?>,
				...
				google.maps.event.addDomListener(window, 'load', oxo_run_map_<?php echo $map_id ; ?>);
			</script>
			<?php
			if( $defaults['id'] ) {
				$html = ob_get_clean() . sprintf( '<div id="%s"><div %s></div></div>', $defaults['id'], OxoCore_Plugin::attributes( 'google-map-shortcode' ) );
			} else {
				$html = ob_get_clean() . sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'google-map-shortcode' ) );
			}
	...
	function attr() {
		...
		$attr['class'] = 'shortcode-map oxo-google-map';
		...
		if ( $force_refresh || 
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
				  	...
				  	return __( 'No location found for the entered address.', 'oxo-core' );
				} elseif( $data->status === 'INVALID_REQUEST' ) {
				   	return __( 'Invalid request. Did you enter an address?', 'oxo-core' );
				} else {
					return __( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'oxo-core' );
				}

			} else {
			 	return __( 'Unable to contact Google API service.', 'oxo-core' );
			}
