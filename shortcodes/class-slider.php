...
class OxoSC_Slider {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_slider-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_slider-shortcode-slide-link', array( $this, 'slide_link_attr' ) );
		add_filter( 'oxo_attr_slider-shortcode-slide-li', array( $this, 'slide_li_attr' ) );	
		add_filter( 'oxo_attr_slider-shortcode-slide-img', array( $this, 'slide_img_attr' ) );
		add_filter( 'oxo_attr_slider-shortcode-slide-img-wrapper', array( $this, 'slide_img_wrapper_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<div %s><ul %s>%s</ul></div>', OxoCore_Plugin::attributes( 'slider-shortcode' ), OxoCore_Plugin::attributes( 'slides' ), do_shortcode( $content ) );
	...
	function attr() {
		...
		$attr['class'] = 'oxo-slider-sc flexslider';
	...
	function render_child( $args, $content = '') {
		...
		if( $type == 'image' ) {
			...
			if( ! empty( $link ) && $link ) {
				$image_id = OxoCore_Plugin::get_attachment_id_from_url( $link );
			} else {
				$image_id = OxoCore_Plugin::get_attachment_id_from_url( $src );
			}
		...
		$html = sprintf( '<li %s>', OxoCore_Plugin::attributes( 'slider-shortcode-slide-li' ) );

		if( $link && 
			! empty( $link )
		) {
			$html .= sprintf( '<a %s>', OxoCore_Plugin::attributes( 'slider-shortcode-slide-link' ) );
		}
		
		if( ! empty( $type ) && 
			$type == 'video'
		) {
			$html .= sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'full-video' ), do_shortcode($content) );
		} else {
			$html .= sprintf( '<span %s><img %s /></span>', OxoCore_Plugin::attributes( 'slider-shortcode-slide-img-wrapper' ), OxoCore_Plugin::attributes( 'slider-shortcode-slide-img' ) );
		}
	...
	function slide_link_attr() {
		...
		$image_id = OxoCore_Plugin::get_attachment_id_from_url( self::$child_args['link'] );
