...
class OxoSC_Person {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_person-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_person-shortcode-image-container', array( $this, 'image_container_attr' ) );
		add_filter( 'oxo_attr_person-shortcode-href', array( $this, 'href_attr' ) );
		add_filter( 'oxo_attr_person-shortcode-img', array( $this, 'img_attr' ) );
		add_filter( 'oxo_attr_person-shortcode-author', array( $this, 'author_attr' ) );
		add_filter( 'oxo_attr_person-shortcode-social-networks', array( $this, 'social_networks_attr' ) );	
		add_filter( 'oxo_attr_person-shortcode-icon', array( $this, 'icon_attr' ) );	
		add_filter( 'oxo_attr_person-desc', array( $this, 'desc_attr' ) );	
	...
	function render( $args, $content = '') {
		...
		$rgb = OxoCore_Plugin::hex2rgb( $defaults['pic_style_color'] );
		...
		if( $picture ) {
			$picture = sprintf( '<img %s />', OxoCore_Plugin::attributes( 'person-shortcode-img' ) ) ;
		
			if ( $pic_link ) {
				$picture = sprintf( '<a %s>%s</a>', OxoCore_Plugin::attributes( 'person-shortcode-href' ), $picture ) ;
			}
			
			$picture =  sprintf( '<div %s><div %s>%s</div></div>', OxoCore_Plugin::attributes( 'person-shortcode-image-wrapper' ), 
								 OxoCore_Plugin::attributes( 'person-shortcode-image-container' ), $picture ) ;
		...
		if( $name || $title || $content ) {
			...
			//$social_networks = OxoCore_Plugin::order_array_like_array( $social_networks, $social_icon_order );
			...
			foreach( $social_networks as $network => $link ) {
				...
				$icons .= sprintf( '<a %s></a>', OxoCore_Plugin::attributes( 'person-shortcode-icon', $icon_options ) );
				$i++;
			}
			
			if( count( $social_networks ) > 0 ) {
				$social_icons_content_top = sprintf( '<div %s><div %s>%s</div></div>', OxoCore_Plugin::attributes( 'person-shortcode-social-networks' ), OxoCore_Plugin::attributes( 'oxo-social-networks-wrapper' ), $icons );
				$social_icons_content_bottom = sprintf( '<div %s><div %s>%s</div></div>', OxoCore_Plugin::attributes( 'person-shortcode-social-networks' ), OxoCore_Plugin::attributes( 'oxo-social-networks-wrapper' ), $icons );
			}
			...
			$person_author_wrapper = sprintf( '<div %s><span %s>%s</span><span %s>%s</span></div>', OxoCore_Plugin::attributes( 'person-author-wrapper' ), 
											  OxoCore_Plugin::attributes( 'person-name' ), $name, OxoCore_Plugin::attributes( 'person-title' ), $title );
			...
			$inner_content .= sprintf( '<div %s><div %s>%s</div><div %s>%s</div>%s</div>', OxoCore_Plugin::attributes( 'person-desc' ), 
										OxoCore_Plugin::attributes( 'person-shortcode-author' ), $person_author_content, 
										OxoCore_Plugin::attributes( 'person-content oxo-clearfix' ), do_shortcode( $content ), $social_icons_content_bottom );				

		}
		
		$html = sprintf( '<div %s>%s%s</div>', OxoCore_Plugin::attributes( 'person-shortcode' ), $picture, $inner_content );
	...
	function attr() {
		...
		$attr['class'] = 'oxo-person person';
		$attr['class'] .= ' oxo-person-' . self::$args['content_alignment'];
		$attr['class'] .= ' oxo-person-icon-' . self::$args['icon_position'];
