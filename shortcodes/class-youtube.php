...
class OxoSC_Youtube {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_youtube-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_youtube-shortcode-video-sc', array( $this, 'video_sc_attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		$html = sprintf( '<div %s><div %s><iframe title="YouTube video player" src="%s://www.youtube.com/embed/%s?wmode=transparent&autoplay=0%s" width="%s" height="%s" frameborder="0" allowfullscreen></iframe></div></div>',
						 OxoCore_Plugin::attributes( 'youtube-shortcode' ), OxoCore_Plugin::attributes( 'youtube-shortcode-video-sc' ), $protocol, $id, $api_params, $width, $height );
	...
	function attr() {
		...
		$attr['class'] = 'oxo-video oxo-youtube';
