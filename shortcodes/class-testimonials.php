...
class OxoSC_Testimonials {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_testimonials-shortcode', array( $this, 'attr' ) );
		add_filter( 'oxo_attr_testimonials-shortcode-testimonials', array( $this, 'testimonials_attr' ) );
		add_filter( 'oxo_attr_testimonials-shortcode-quote', array( $this, 'quote_attr' ) );		
		add_filter( 'oxo_attr_testimonials-shortcode-blockquote', array( $this, 'blockquote_attr' ) );	
		add_filter( 'oxo_attr_testimonials-shortcode-review', array( $this, 'review_attr' ) );		
		add_filter( 'oxo_attr_testimonials-shortcode-thumbnail', array( $this, 'thumbnail_attr' ) );
		add_filter( 'oxo_attr_testimonials-shortcode-image', array( $this, 'image_attr' ) );
		add_filter( 'oxo_attr_testimonials-shortcode-author', array( $this, 'author_attr' ) );
		add_filter( 'oxo_attr_testimonials-shortcode-pagination', array( $this, 'pagination_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		#oxo-testimonials-%s a{border-color: {$textcolor};}#oxo-testimonials-%s a:hover, #oxo-testimonials-%s .activeSlide{background-color: {$textcolor};}
		.oxo-testimonials.%s.oxo-testimonials-%s .author:after{border-top-color:{$backgroundcolor} !important;}</style>", 
		...
		if( self::$parent_args['design'] == 'clean' ) {
			$pagination  = sprintf( '<div %s></div>', OxoCore_Plugin::attributes( 'testimonials-shortcode-pagination' ) );
		}

		$html = sprintf( '<div %s>%s<div %s>%s</div>%s</div>', OxoCore_Plugin::attributes( 'testimonials-shortcode' ), $styles,
						 OxoCore_Plugin::attributes( 'testimonials-shortcode-testimonials' ), do_shortcode($content), $pagination );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'oxo-testimonials %s oxo-testimonials-%s', self::$parent_args['design'], $this->testimonials_counter );
	...
	private function render_child_classic( $content ) {
		...
		if( self::$child_args['name'] ) {
			...
			if( self::$child_args['avatar'] == 'image' && 
				...
				$image_id = OxoCore_Plugin::get_attachment_id_from_url( self::$child_args['image'] );
				self::$child_args['alt'] = '';
				if( $image_id ) {
					self::$child_args['alt'] = get_post_field( 'post_excerpt', $image_id );
				}

				$pic = sprintf( '<img %s />', OxoCore_Plugin::attributes( 'testimonials-shortcode-image' ) );
			}
			...
			if( self::$child_args['avatar'] != 'none' ) {
				$thumbnail = sprintf( '<span %s>%s</span>', OxoCore_Plugin::attributes( 'testimonials-shortcode-thumbnail' ), $pic );
			}
			
			$inner_content .= sprintf( '<div %s>%s<span %s>', OxoCore_Plugin::attributes( 'testimonials-shortcode-author' ), $thumbnail, OxoCore_Plugin::attributes( 'company-name' ) );
		...
		$html = sprintf( '<div %s><blockquote><q %s>%s</q></blockquote>%s</div>', OxoCore_Plugin::attributes( 'testimonials-shortcode-review' ), 
						 OxoCore_Plugin::attributes( 'testimonials-shortcode-quote' ), do_shortcode( $content ), $inner_content );
	...
	private function render_child_clean( $content ) {
		...
		if( self::$child_args['avatar'] != 'none' ) {
			$thumbnail = sprintf( '<div %s>%s</div>', OxoCore_Plugin::attributes( 'testimonials-shortcode-thumbnail' ), $pic );
		}	
	
		$author .= sprintf( '<div %s><span %s>', OxoCore_Plugin::attributes( 'testimonials-shortcode-author' ), OxoCore_Plugin::attributes( 'company-name' ) );	
		...
		$html = sprintf( '<div %s>%s<blockquote %s><q %s>%s</q></blockquote>%s</div>', OxoCore_Plugin::attributes( 'testimonials-shortcode-review' ), $thumbnail,
						 OxoCore_Plugin::attributes( 'testimonials-shortcode-blockquote' ), OxoCore_Plugin::attributes( 'testimonials-shortcode-quote' ), 
	...
	function pagination_attr() {
		...
		$attr['id'] = sprintf( 'oxo-testimonials-%s', $this->testimonials_counter );
